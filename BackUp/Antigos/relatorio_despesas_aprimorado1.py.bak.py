...
##            print("=== FIM OBTER ACUMULADO ===\n")
    ...
    def criar_tabela_despesas(self, dados, colunas, larguras, incluir_total=True):
        ...
        colunas_numericas = ['VALOR', 'TOTAL', 'SALÁRIO/ADIANTAMENTO', 'RESCISÃO', '13º SALÁRIO', 
        ...
        if incluir_total:
            ...
            if coluna_valor >= 0:
                ...
                if 'SALÁRIO/FÉRIAS' in colunas and 'RESCISÃO/13º SALÁRIO' in colunas:
...
##                    print("Tabela adicionada com sucesso")
    ...
    def adicionar_detalhes(self, elementos, dados):
        ...
        # 1.A) Despesas com Colaboradores - Salários, Transportes e Café
        if not dados['df_tp_desp_1'].empty:
            # Filtrar os lançamentos que NÃO são FÉRIAS, RESCISÃO ou 13º SALÁRIO
            df_salarios = dados['df_tp_desp_1'][
                ~dados['df_tp_desp_1']['REFERÊNCIA'].isin(['FÉRIAS', 'RESCISÃO', '13º SALÁRIO'])
            ].copy()
            
            if not df_salarios.empty:
                logger.debug("Processando despesas com colaboradores - salários e adiantamentos")
                elementos.append(Paragraph("1) DESPESAS COM COLABORADORES - SALÁRIOS E ADIANTAMENTOS", 
                                    self.config.style_despesa))
                
                # Função para consolidar lançamentos de salário
                def consolidar_salarios(df):
                    df = df.copy()
                    df = df.fillna("")
                    df = df.infer_objects(copy=False)
                    ...
                    for nome, grupo in df.groupby('NOME'):
                        ...
                        # Salário/Adiantamento (tudo que não é FÉRIAS, RESCISÃO, 13º)
                        salario_refs = [ref for ref in grupo['REFERÊNCIA'].unique() 
                                    if ref not in ['FÉRIAS', 'RESCISÃO', '13º SALÁRIO', 'TRANSPORTE', 'CAFÉ']]
                        
                        salario_valor = 0
                        for ref in salario_refs:
                            valores = grupo[grupo['REFERÊNCIA'] == ref]['VALOR']
                            salario_valor += pd.to_numeric(valores, errors='coerce').sum()
                        
                        linha['SALÁRIO/ADIANTAMENTO'] = salario_valor
                        
                        # Transporte
                        transporte_valores = grupo[grupo['REFERÊNCIA'] == 'TRANSPORTE']['VALOR']
                        linha['TRANSPORTE'] = pd.to_numeric(transporte_valores, errors='coerce').sum() if not transporte_valores.empty else 0
                        
                        # Café
                        cafe_valores = grupo[grupo['REFERÊNCIA'] == 'CAFÉ']['VALOR']
                        linha['CAFÉ'] = pd.to_numeric(cafe_valores, errors='coerce').sum() if not cafe_valores.empty else 0
                        ...
                        linha['DADOS BANCÁRIOS'] = grupo['DADOS BANCÁRIOS'].iloc[0] if 'DADOS BANCÁRIOS' in grupo.columns and not grupo['DADOS BANCÁRIOS'].empty else ''
                        
                        # CORREÇÃO: Calcular o total corretamente
                        linha['TOTAL'] = linha['SALÁRIO/ADIANTAMENTO'] + linha['TRANSPORTE'] + linha['CAFÉ']
                    ...
                    # Definir ordem das colunas
                    colunas_ordem = ['NOME', 'SALÁRIO/ADIANTAMENTO', 'DIAS', 
                                'TRANSPORTE', 'CAFÉ', 'TOTAL', 'DADOS BANCÁRIOS']
                    
                    df_result = df_result.reindex(columns=colunas_ordem)
                    
                    # Ordenar por SALÁRIO/ADIANTAMENTO (maior para menor)
                    df_result = df_result.sort_values(by='SALÁRIO/ADIANTAMENTO', ascending=False)
                    
                    return df_result
                
                # Consolidar e criar tabela
                df_consolidado = consolidar_salarios(df_salarios)
                logger.debug(f"Total de funcionários processados (salários): {len(df_consolidado)}")
                
                # Colunas a serem formatadas e alinhadas à direita
                colunas_numericas = ['SALÁRIO/ADIANTAMENTO', 'TRANSPORTE', 'CAFÉ', 'TOTAL']
                
                # Formatar valores e garantir formato correto
                df_formatado = df_consolidado.copy()
                for coluna in colunas_numericas:
                    # Garantir que os valores são numéricos antes de formatar
                    df_formatado[coluna] = pd.to_numeric(df_formatado[coluna], errors='coerce').fillna(0)
                    df_formatado[coluna] = df_formatado[coluna].apply(
                        lambda x: self.formatar_numero(x) if pd.notnull(x) else "0,00"
                    )
                
                # Definir larguras de colunas
                larguras = [220, 70, 40, 70, 70, 70, 220]
                
                # Criando a tabela com alinhamento à direita para colunas numéricas
                tabela = Table(
                    [df_formatado.columns.tolist()] + df_formatado.values.tolist(),
                    colWidths=larguras
                )
                ...
                # Alinhar valores à direita para colunas numéricas
                for i, col in enumerate(df_formatado.columns):
                    if col in colunas_numericas or col == 'DIAS':
                        estilos.append(('ALIGN', (i, 0), (i, -1), 'RIGHT'))
                
                # Adicionar linha de total
                if len(df_formatado) > 0:
                    # Calcular valores totais
                    totais = {}
                    for col in colunas_numericas:
                        total_valor = df_consolidado[col].sum()
                        totais[col] = self.formatar_numero(total_valor)
                    
                    # Adicionar linha de subtotal
                    linha_total = ['Subtotal', '', '', '', '', '', '']
                    for i, col in enumerate(df_formatado.columns):
                        if col in totais:
                            linha_total[i] = totais[col]
                    
                    tabela_data = [df_formatado.columns.tolist()] + df_formatado.values.tolist() + [linha_total]
                    tabela = Table(tabela_data, colWidths=larguras)
                    
                    # Adicionar estilos para a linha de total
                    estilos.extend([
                        ('FONTNAME', (0, -1), (-1, -1), 'Helvetica-Bold'),
                        ('BACKGROUND', (0, -1), (-1, -1), colors.lightgrey),
                    ])
            ...
            # 1.B) Despesas com Colaboradores - Férias, Rescisão e 13º
            df_ferias_rescisao = dados['df_tp_desp_1'][
                dados['df_tp_desp_1']['REFERÊNCIA'].isin(['FÉRIAS', 'RESCISÃO', '13º SALÁRIO'])
            ].copy()
            
            if not df_ferias_rescisao.empty:
                logger.debug("Processando despesas com colaboradores - férias, rescisão e 13º")
                elementos.append(Paragraph("1) DESPESAS COM COLABORADORES - FÉRIAS, RESCISÃO E 13º SALÁRIO", 
                                    self.config.style_despesa))
                
                # Função para consolidar lançamentos de férias, rescisão e 13º
                def consolidar_ferias_rescisao(df):
                    df = df.copy()
                    df = df.fillna("")
                    df = df.infer_objects(copy=False)
                    ...
                    for nome, grupo in df.groupby('NOME'):
                        ...
                        # Férias
                        ferias_valores = grupo[grupo['REFERÊNCIA'] == 'FÉRIAS']['VALOR']
                        linha['FÉRIAS'] = pd.to_numeric(ferias_valores, errors='coerce').sum() if not ferias_valores.empty else 0
                        
                        # Rescisão
                        rescisao_valores = grupo[grupo['REFERÊNCIA'] == 'RESCISÃO']['VALOR']
                        linha['RESCISÃO'] = pd.to_numeric(rescisao_valores, errors='coerce').sum() if not rescisao_valores.empty else 0
                        
                        # 13º Salário
                        decimo_valores = grupo[grupo['REFERÊNCIA'] == '13º SALÁRIO']['VALOR']
                        linha['13º SALÁRIO'] = pd.to_numeric(decimo_valores, errors='coerce').sum() if not decimo_valores.empty else 0
                        
                        linha['DADOS BANCÁRIOS'] = grupo['DADOS BANCÁRIOS'].iloc[0] if 'DADOS BANCÁRIOS' in grupo.columns and not grupo['DADOS BANCÁRIOS'].empty else ''
                        
                        # CORREÇÃO: Calcular o total corretamente
                        linha['TOTAL'] = linha['FÉRIAS'] + linha['RESCISÃO'] + linha['13º SALÁRIO']
                    ...
                    # Se o DataFrame está vazio, criar com as colunas necessárias
                    if df_result.empty:
                        df_result = pd.DataFrame(columns=['NOME', 'FÉRIAS', 'RESCISÃO', '13º SALÁRIO', 'TOTAL', 'DADOS BANCÁRIOS'])
                    else:
                        # Garantir que todos os valores são numéricos para as colunas de valores
                        colunas_numericas = ['FÉRIAS', 'RESCISÃO', '13º SALÁRIO', 'TOTAL']
                        for coluna in colunas_numericas:
                            if coluna in df_result.columns:
                                df_result[coluna] = pd.to_numeric(df_result[coluna], errors='coerce').fillna(0)
                    
                    # Definir ordem das colunas
                    colunas_ordem = ['NOME', 'FÉRIAS', 'RESCISÃO', '13º SALÁRIO', 'TOTAL', 'DADOS BANCÁRIOS']
                    
                    df_result = df_result.reindex(columns=colunas_ordem)
                    
                    # Ordenar por 13º SALÁRIO (maior para menor)
                    if '13º SALÁRIO' in df_result.columns and not df_result.empty:
                        df_result = df_result.sort_values(by='13º SALÁRIO', ascending=False)
                    
                    return df_result
                
                # Consolidar e criar tabela
                df_consolidado = consolidar_ferias_rescisao(df_ferias_rescisao)
                logger.debug(f"Total de funcionários processados (férias/rescisão): {len(df_consolidado)}")
        ...
        # 1.C) Despesas com Colaboradores - Diaristas - Permanece sem alterações significativas
        if not dados['df_diaria'].empty:
            ...
            # Aplicar formatação para valores numéricos
            colunas_numericas = ['DIÁRIA', 'TOTAL']
            for col in colunas_numericas:
                if col in df_diaria_formatado.columns:
                    df_diaria_formatado[col] = pd.to_numeric(df_diaria_formatado[col], errors='coerce').fillna(0)
                    df_diaria_formatado[col] = df_diaria_formatado[col].apply(
                        lambda x: self.formatar_numero(x) if pd.notnull(x) else "0,00"
                    )
            
            # Criar tabela
            colunas = ['NOME', 'DIÁRIA', 'DIAS', 'TOTAL', 'DADOS BANCÁRIOS']
            larguras = [284, 80, 50, 90, 280]
            
            tabela = Table(
                [colunas] + df_diaria_formatado[colunas].values.tolist(),
                colWidths=larguras
            )
            ...
            # Linha de total
            if len(df_diaria_formatado) > 0:
                total_valor = pd.to_numeric(dados['df_diaria']['VALOR'], errors='coerce').sum()
                linha_total = ['Subtotal', '', '', self.formatar_numero(total_valor), '']
                
                tabela_data = [colunas] + df_diaria_formatado[colunas].values.tolist() + [linha_total]
                tabela = Table(tabela_data, colWidths=larguras)
        ...
        # Resto do método (outras despesas) - mantido praticamente sem alterações
        for tipo in range(2, 8):
            ...
            if not df_tipo.empty:
                ...
                # Aplicar formatação para valores numéricos
                if 'VALOR' in df_tipo.columns:
                    df_tipo['VALOR'] = pd.to_numeric(df_tipo['VALOR'], errors='coerce').fillna(0)
                    df_tipo['VALOR'] = df_tipo['VALOR'].apply(
                        lambda x: self.formatar_numero(x) if pd.notnull(x) else "0,00"
                    )
                
                # Criar tabela
                colunas = ['NOME', 'VENCIMENTO', 'REFERÊNCIA', 'VALOR', 'DADOS BANCÁRIOS']
                larguras = [220, 70, 250, 80, 170]
                
                tabela = Table(
                    [colunas] + df_tipo[colunas].values.tolist(),
                    colWidths=larguras
                )
                ...
                valor_idx = colunas.index('VALOR')
                estilos.append(('ALIGN', (valor_idx, 0), (valor_idx, -1), 'RIGHT'))
                
                # Linha de total
                if len(df_tipo) > 0:
                    total_valor = pd.to_numeric(dados['df_filtrado'].loc[dados['df_filtrado']['TP_DESP'] == tipo, 'VALOR'], 
                                            errors='coerce').sum()
                    linha_total = ['', '', 'Subtotal', self.formatar_numero(total_valor), '']
                    
                    tabela_data = [colunas] + df_tipo[colunas].values.tolist() + [linha_total]
                    tabela = Table(tabela_data, colWidths=larguras)
