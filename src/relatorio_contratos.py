...
# Importar bibliotecas com tratamento de exceções
try:
    from dateutil.relativedelta import relativedelta
except ImportError:
    # Criar uma alternativa simples para relativedelta se não estiver disponível
    class relativedelta:
...
try:
    ...
    print("Aviso: pandas não está instalado. Alguns recursos estarão indisponíveis.")
...
try:
    ...
    print("Aviso: openpyxl não está instalado. A exportação para Excel estará indisponível.")
...
try:
    ...
    print("Aviso: matplotlib não está instalado. Os gráficos estarão indisponíveis.")
...
try:
    ...
    def configurar_janela(janela, titulo="Janela", largura=800, altura=1000):
        janela.title(titulo)
...
class RelatorioContratos:
    ...
    def __init__(self, parent=None):
        ...
        configurar_janela(self.root, "Relatório de Contratos", 1000, 1200)
    ...
    # Substitua COMPLETAMENTE o método setup_gui por esta versão que usa grid

    def setup_gui(self):
        """Configuração da interface gráfica principal usando grid para controle preciso"""
        # Configure a raiz para usar grid
        self.root.grid_rowconfigure(0, weight=1)
        self.root.grid_columnconfigure(0, weight=1)
        
        # Frame principal que conterá tudo (usando grid)
        self.frame_principal = ttk.Frame(self.root, padding=10)
        self.frame_principal.grid(row=0, column=0, sticky='nsew')
        
        # Configure o grid para o frame principal
        self.frame_principal.grid_rowconfigure(2, weight=1)  # A row dos resultados expandirá
        self.frame_principal.grid_columnconfigure(0, weight=1)
        
        # Frame para seleção - row 0
        self.frame_selecao = ttk.LabelFrame(self.frame_principal, text="Seleção de Cliente e Data")
        self.frame_selecao.grid(row=0, column=0, sticky='ew', pady=5)
        
        # Configurar grid para o frame de seleção
        self.frame_selecao.grid_columnconfigure(1, weight=1)
        
        # Widgets da seleção - usando grid para posicionamento preciso
        ttk.Label(self.frame_selecao, text="Selecione o Cliente:", font=('Arial', 11)).grid(row=0, column=0, padx=5, pady=5, sticky='w')
        self.cliente_combobox = ttk.Combobox(self.frame_selecao, width=40, font=('Arial', 11))
        ...
        ttk.Label(self.frame_selecao, text="Data de Referência:", font=('Arial', 11)).grid(row=1, column=0, padx=5, pady=5, sticky='w')
        ...
        # Botão de gerar relatório - colocado na mesma linha da data
        style = ttk.Style()
        ...
        # Frame para resultados - row 2 (com peso 1 para expandir)
        self.frame_resultados = ttk.LabelFrame(self.frame_principal, text="Resultados")
        self.frame_resultados.grid(row=2, column=0, sticky='nsew', pady=5)
        
        # Configure o grid para frame_resultados
        ...
        # Adicionar aba de gráfico apenas se matplotlib estiver disponível
        if MATPLOTLIB_AVAILABLE:
            self.aba_grafico = ttk.Frame(self.notebook)
            self.notebook.add(self.aba_grafico, text='Gráfico')
        
        # Frame para botões - row 3 (fixo na parte inferior)
        self.frame_botoes = ttk.Frame(self.frame_principal)
        self.frame_botoes.grid(row=3, column=0, sticky='ew', pady=10)
        
        # Configure grid para frame_botoes
        self.frame_botoes.grid_columnconfigure(0, weight=1)  # Para empurrar os botões para lados opostos
        ...
        try:
            ...
            # Caso a variável OPENPYXL_AVAILABLE não esteja definida
            ttk.Button(
                self.frame_botoes,
                text="Exportar para Excel",
                command=self.exportar_excel
...
class RelatorioContratos:
    ...
    def __init__(self, parent=None):
        ...
        configurar_janela(self.root, "Relatório de Contratos", 1000, 700)
