            ...
            if 'Contratos_ADM' in workbook.sheetnames:
                ...
                for row in sheet.iter_rows(min_row=2, values_only=True):
                    if row[0] and str(row[0]) == str(num_contrato):
                        # Formatar CNPJ/CPF
                        cnpj_cpf = formatar_cnpj_cpf(row[7]) if row[7] else ""
                        
                        # Determinar tipo de valor (percentual ou fixo)
                        # Verificar se valor[10] é string antes de procurar "%" nele
                        tipo = "Percentual" if (row[10] and isinstance(row[10], str) and "%" in row[10]) else "Valor Fixo"
                        
                        # Formatar valor com tratamento de tipo
                        valor = ""
                        if tipo == "Percentual" and row[9] and isinstance(row[9], str):
                            valor = row[10].replace("%", "") + "%"
                        else:
                            if row[10]:
                                try:
                                    if isinstance(row[11], str):
                                        valor_formatado = float(row[11].replace(',', '.'))
                                        valor = f"R$ {valor_formatado:,.2f}"
                                    else:
                                        valor = f"R$ {float(row[11]):,.2f}"
                                except (ValueError, TypeError):
                                    valor = f"R$ {str(row[11])}"
                            else:
                                valor = "R$ 0,00"
                        ...
                        if row[11]:
                            try:
                                if isinstance(row[11], str):
                                    ...
                                    valor_total = f"R$ {float(row[11]):,.2f}"
                            except (ValueError, TypeError):
                                valor_total = f"R$ {str(row[11])}"
                        else:
                            valor_total = "R$ 0,00"
                        ...
                        if row[8]:
                            # Garantir que admin_nome seja uma string
                            if isinstance(row[8], datetime):
                                admin_nome = row[8].strftime("%d/%m/%Y")
                            else:
                                admin_nome = str(row[8])
                        else:
                            admin_nome = "Sem nome"
