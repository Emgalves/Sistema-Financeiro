...
class SistemaEntradaDados:
    ...
    # def cadastrar_material_vinculado_nf(self):
    #     """Cadastra material com dados da NF já preenchidos"""
        
    #     # Abrir janela de cadastro de material
    #     self.novo_material_direto()
        
    #     # Preencher campos automaticamente com dados atuais da tela (não da lista)
    #     try:
    #         # Marcar que tem dados de compra
    #         if hasattr(self, 'tem_dados_compra'):
    #             self.tem_dados_compra.set(True)
    #             self.toggle_campos_compra_material()
            
    #         # Preencher campos com dados da tela atual
    #         if hasattr(self, 'campos_material') and self.campos_material:
                
    #             # Nome do fornecedor
    #             if 'fornecedor' in self.campos_material:
    #                 nome_fornecedor = self.campos_fornecedor['nome'].get()
    #                 if nome_fornecedor:
    #                     self.campos_material['fornecedor'].delete(0, tk.END)
    #                     self.campos_material['fornecedor'].insert(0, nome_fornecedor)
                
    #             # CNPJ do fornecedor
    #             if 'cnpj_fornecedor' in self.campos_material:
    #                 cnpj_fornecedor = self.campos_fornecedor['cnpj_cpf'].get()
    #                 if cnpj_fornecedor:
    #                     self.campos_material['cnpj_fornecedor'].delete(0, tk.END)
    #                     self.campos_material['cnpj_fornecedor'].insert(0, cnpj_fornecedor)
                
    #             # Data da compra (usar data do relatório)
    #             if 'data_compra' in self.campos_material:
    #                 data_relatorio = self.data_rel_entry.get()
    #                 if data_relatorio:
    #                     self.campos_material['data_compra'].delete(0, tk.END)
    #                     self.campos_material['data_compra'].insert(0, data_relatorio)
                
    #             # Número da NF
    #             if 'nf' in self.campos_material:
    #                 numero_nf = self.campos_despesa['nf'].get()
    #                 if numero_nf:
    #                     self.campos_material['nf'].delete(0, tk.END)
    #                     self.campos_material['nf'].insert(0, numero_nf)
                
    #             # Valor total (se estiver calculado)
    #             if 'valor_total' in self.campos_material:
    #                 valor_total = self.campos_despesa['valor'].get()
    #                 if valor_total:
    #                     self.campos_material['valor_total'].delete(0, tk.END)
    #                     self.campos_material['valor_total'].insert(0, valor_total)
                
    #             print("DEBUG: Campos de material preenchidos com sucesso!")
    #         else:
    #             print("DEBUG: campos_material não encontrado ou vazio")
                
    #     except Exception as e:
    #         print(f"DEBUG: Erro ao preencher campos de material: {str(e)}")
    #         # Não mostrar erro pro usuário, apenas continuar com cadastro vazio

    def cadastrar_material_vinculado_nf(self):
        """Cadastra material com dados da NF já preenchidos"""
        try:
            print("🔗 Abrindo cadastro de material vinculado à NF...")
            
            # Abrir janela de cadastro
            self.novo_material_direto()
            
            # Aguardar criação da janela
            self.janela_material.after(100, self.preencher_dados_nf)
            
        except Exception as e:
            print(f"❌ Erro ao abrir material vinculado: {e}")
            custom_messagebox("error", "Erro", f"Erro ao abrir cadastro: {str(e)}")

    def preencher_dados_nf(self, dados_nf=None):
        """
        Preenche campos de material com dados da NF
        
        Args:
            dados_nf (dict, optional): Dados específicos da NF. 
                                    Se None, usa dados da tela atual.
        """
        try:
            # Marcar que tem dados de compra
            if hasattr(self, 'tem_dados_compra'):
            ...
            if dados_nf is not None:
                # MODO 1: Usar dados fornecidos via parâmetro
                print("📋 Preenchendo com dados fornecidos...")
                
                if 'nome' in dados_nf and dados_nf['nome']:
                    self.campos_material['fornecedor'].delete(0, tk.END)
                    self.campos_material['fornecedor'].insert(0, dados_nf['nome'])
                
                if 'cnpj_cpf' in dados_nf and dados_nf['cnpj_cpf']:
                    self.campos_material['cnpj_fornecedor'].delete(0, tk.END)
                    self.campos_material['cnpj_fornecedor'].insert(0, dados_nf['cnpj_cpf'])
                
                if 'data' in dados_nf and dados_nf['data']:
                    self.campos_material['data_compra'].delete(0, tk.END)
                    self.campos_material['data_compra'].insert(0, dados_nf['data'])
                
                if 'nf' in dados_nf and dados_nf['nf']:
                    self.campos_material['nf'].delete(0, tk.END)
                    self.campos_material['nf'].insert(0, dados_nf['nf'])
                
                if 'valor' in dados_nf and dados_nf['valor']:
                    self.campos_material['valor_total'].delete(0, tk.END)
                    self.campos_material['valor_total'].insert(0, dados_nf['valor'])
                    
            else:
                # MODO 2: Usar dados da tela atual
                print("📋 Preenchendo com dados da tela atual...")
                
                if hasattr(self, 'campos_fornecedor') and hasattr(self, 'campos_despesa'):
            ...
            print("✅ Dados da NF preenchidos automaticamente!")
            
        except Exception as e:
            print(f"❌ Erro ao preencher dados da NF: {e}")

    def preencher_dados_nf_na_interface(self):
        """Preenche dados da NF na interface de materiais"""
        try:
            # Marcar que tem dados de compra
            self.tem_dados_compra.set(True)
            self.toggle_dados_fornecedor()
            
            # Preencher campos do fornecedor com dados da tela atual
            if hasattr(self, 'campos_fornecedor') and hasattr(self, 'campos_despesa'):
                
                # Nome do fornecedor
                nome_fornecedor = self.campos_fornecedor['nome'].get()
                if nome_fornecedor:
                    self.campos_fornecedor_fixo['fornecedor'].delete(0, tk.END)
                    self.campos_fornecedor_fixo['fornecedor'].insert(0, nome_fornecedor)
                
                # CNPJ do fornecedor
                cnpj_fornecedor = self.campos_fornecedor['cnpj_cpf'].get()
                if cnpj_fornecedor:
                    self.campos_fornecedor_fixo['cnpj'].delete(0, tk.END)
                    self.campos_fornecedor_fixo['cnpj'].insert(0, cnpj_fornecedor)
                
                # Data da compra (usar data do relatório)
                data_relatorio = self.data_rel_entry.get()
                if data_relatorio:
                    self.campos_fornecedor_fixo['data_compra'].delete(0, tk.END)
                    self.campos_fornecedor_fixo['data_compra'].insert(0, data_relatorio)
                
                # Número da NF
                numero_nf = self.campos_despesa['nf'].get()
                if numero_nf:
                    self.campos_fornecedor_fixo['nf'].delete(0, tk.END)
                    self.campos_fornecedor_fixo['nf'].insert(0, numero_nf)
                
                print("✅ Dados da NF preenchidos na interface de materiais!")
                custom_messagebox("info", "Informação", 
                    "Dados da NF foram preenchidos automaticamente!\nAgora adicione os materiais clicando em 'Novo Material'.")
            
        except Exception as e:
            print(f"❌ Erro ao preencher dados da NF: {e}")

    def inicializar_sistema_materiais(self):
        """Inicializa o sistema de materiais - CHAMAR no final do setup_aba_fornecedor()"""
        try:
            print("🔧 Inicializando sistema de materiais...")
            
            # Inicializar gerenciador se não existir
            if not hasattr(self, 'gerenciador_materiais'):
                print("📦 Criando gerenciador de materiais...")
                self.gerenciador_materiais = GerenciadorMateriais(self)
                self.gerenciador_materiais.inicializar_planilha_materiais()
            
            # Adicionar seção de materiais na aba fornecedor
            print("🔨 Adicionando seção de materiais...")
            self.adicionar_secao_materiais_fornecedor()
    ...
    def adicionar_secao_materiais_fornecedor(self):
        """Adiciona seção de materiais na aba fornecedor existente"""
        try:
            # Adicionar ANTES do último separador
            frame_materiais = ttk.LabelFrame(self.aba_fornecedor, text="🏗️ Gestão de Materiais da Obra")
            frame_materiais.pack(fill='x', padx=10, pady=5)
            
            # Container para botões de materiais
            frame_botoes_materiais = ttk.Frame(frame_materiais)
            frame_botoes_materiais.pack(fill='x', padx=5, pady=8)
            
            # Botões de materiais
            ttk.Button(
                frame_botoes_materiais,
                text="📦 Novo Material",
                command=self.novo_material_direto,
                style='Medium.TButton'
            ).pack(side='left', padx=5)
            
            ttk.Button(
                frame_botoes_materiais,
                text="📋 Consultar Materiais",
                command=self.consultar_materiais,
                style='Medium.TButton'
            ).pack(side='left', padx=5)
            ...
            ttk.Button(
                ...
                command=self.gerar_manual_proprietario,
                style='Medium.TButton'
            ).pack(side='left', padx=5)
            ...
            print(f"❌ Erro ao adicionar seção de materiais: {e}")

    def novo_material_direto(self):
        """Abre janela de cadastro de materiais com formulário para cada item"""
        try:
            print("🆕 Abrindo cadastro de materiais com formulário...")
            
            self.janela_material = tk.Toplevel(self.root)
            self.janela_material.title("Cadastro de Materiais da Obra")
            self.janela_material.geometry("1400x800")
            self.janela_material.grab_set()  # Modal
            
            # Lista para armazenar materiais em memória
            self.lista_materiais = []
            
            # Frame principal
            frame_principal = ttk.Frame(self.janela_material)
            frame_principal.pack(fill='both', expand=True, padx=10, pady=10)
            
            # ========== SEÇÃO 1: DADOS DO FORNECEDOR (FIXOS PARA TODOS OS ITENS) ==========
            frame_fornecedor_fixo = ttk.LabelFrame(frame_principal, text="💼 Dados do Fornecedor (aplicados a todos os itens)", padding=10)
            frame_fornecedor_fixo.pack(fill='x', pady=5)
            
            # Checkbox para dados de compra
            self.tem_dados_compra = tk.BooleanVar()
            checkbox_compra = tk.Checkbutton(
                frame_fornecedor_fixo, 
                text="Estes materiais possuem dados de compra/fornecedor", 
                variable=self.tem_dados_compra,
                command=self.toggle_dados_fornecedor,
                font=('Arial', 10, 'bold')
            )
            checkbox_compra.pack(anchor='w', pady=5)
            
            # Frame para dados do fornecedor
            self.frame_dados_fornecedor = ttk.Frame(frame_fornecedor_fixo)
            self.frame_dados_fornecedor.pack(fill='x', pady=5)
            
            # Campos do fornecedor em layout horizontal
            self.campos_fornecedor_fixo = {}
            
            # Linha 1: Fornecedor e CNPJ
            tk.Label(self.frame_dados_fornecedor, text="Fornecedor:", font=('Arial', 9)).grid(row=0, column=0, sticky='w', padx=5, pady=5)
            self.campos_fornecedor_fixo['fornecedor'] = tk.Entry(self.frame_dados_fornecedor, width=40, state='disabled')
            self.campos_fornecedor_fixo['fornecedor'].grid(row=0, column=1, sticky='ew', padx=5, pady=5)
            
            tk.Label(self.frame_dados_fornecedor, text="CNPJ/CPF:", font=('Arial', 9)).grid(row=0, column=2, sticky='w', padx=5, pady=5)
            self.campos_fornecedor_fixo['cnpj'] = tk.Entry(self.frame_dados_fornecedor, width=20, state='disabled')
            self.campos_fornecedor_fixo['cnpj'].grid(row=0, column=3, sticky='ew', padx=5, pady=5)
            
            # Linha 2: Data e NF
            tk.Label(self.frame_dados_fornecedor, text="Data Compra:", font=('Arial', 9)).grid(row=1, column=0, sticky='w', padx=5, pady=5)
            self.campos_fornecedor_fixo['data_compra'] = tk.Entry(self.frame_dados_fornecedor, width=15, state='disabled')
            self.campos_fornecedor_fixo['data_compra'].grid(row=1, column=1, sticky='w', padx=5, pady=5)
            
            tk.Label(self.frame_dados_fornecedor, text="Nº NF:", font=('Arial', 9)).grid(row=1, column=2, sticky='w', padx=5, pady=5)
            self.campos_fornecedor_fixo['nf'] = tk.Entry(self.frame_dados_fornecedor, width=20, state='disabled')
            self.campos_fornecedor_fixo['nf'].grid(row=1, column=3, sticky='ew', padx=5, pady=5)
            
            # Configurar grid
            self.frame_dados_fornecedor.columnconfigure(1, weight=1)
            self.frame_dados_fornecedor.columnconfigure(3, weight=1)
            
            # ========== SEÇÃO 2: LISTA DE MATERIAIS ==========
            frame_lista = ttk.LabelFrame(frame_principal, text="📋 Lista de Materiais", padding=10)
            frame_lista.pack(fill='both', expand=True, pady=5)
            
            # Frame para botões da lista
            frame_botoes_lista = ttk.Frame(frame_lista)
            frame_botoes_lista.pack(fill='x', pady=5)
            
            ttk.Button(frame_botoes_lista, text="➕ Novo Material", 
                    command=self.abrir_formulario_material, style='Medium.TButton').pack(side='left', padx=5)
            ttk.Button(frame_botoes_lista, text="✏️ Editar Selecionado", 
                    command=self.editar_material_selecionado_nova_interface, style='Medium.TButton').pack(side='left', padx=5)
            ttk.Button(frame_botoes_lista, text="📋 Duplicar Selecionado", 
                    command=self.duplicar_material_selecionado, style='Medium.TButton').pack(side='left', padx=5)
            ttk.Button(frame_botoes_lista, text="🗑️ Remover Selecionado", 
                    command=self.remover_material_selecionado, style='Medium.TButton').pack(side='left', padx=5)
            ...
            # TreeView para os materiais (somente leitura)
            colunas = (
                'codigo', 'categoria', 'descricao', 'marca_modelo', 'quantidade', 
                'unidade', 'valor_unit', 'valor_total', 'ambiente', 'status'
            )
            
            self.tree_materiais = ttk.Treeview(frame_scroll, columns=colunas, show='headings',
            ...
            headers_config = {
                ...
                'marca_modelo': ('Marca/Modelo', 150),
                'quantidade': ('Qtd', 60),
                'unidade': ('Un', 50),
                'valor_unit': ('Vlr Unit', 80),
                'valor_total': ('Vlr Total', 80),
                'ambiente': ('Ambiente', 100),
                'status': ('Status', 80)
            }
            
            for col, (header, width) in headers_config.items():
                self.tree_materiais.heading(col, text=header)
                self.tree_materiais.column(col, width=width, minwidth=50)
            
            self.tree_materiais.pack(side='left', fill='both', expand=True)
            
            # Configurar scrollbars
            scroll_y.config(command=self.tree_materiais.yview)
            scroll_x.config(command=self.tree_materiais.xview)
            
            # Bind para duplo-clique editar
            self.tree_materiais.bind('<Double-1>', lambda e: self.editar_material_selecionado_nova_interface())
            
            # ========== RESUMO ==========
            frame_resumo = ttk.LabelFrame(frame_principal, text="📊 Resumo", padding=10)
            frame_resumo.pack(fill='x', pady=5)
            
            self.label_resumo = tk.Label(frame_resumo, text="Nenhum material adicionado", 
                                    font=('Arial', 10))
            self.label_resumo.pack()
            
            # ========== BOTÕES PRINCIPAIS ==========
            frame_botoes_main = ttk.Frame(frame_principal)
            frame_botoes_main.pack(fill='x', pady=10)
            
            ttk.Button(frame_botoes_main, text="💾 Salvar Todos os Materiais", 
                    command=self.salvar_todos_materiais, style='Medium.TButton').pack(side='left', padx=5)
            
            ttk.Button(frame_botoes_main, text="🧹 Limpar Lista", 
                    command=self.limpar_lista_materiais, style='Medium.TButton').pack(side='left', padx=5)
            
            ttk.Button(frame_botoes_main, text="❌ Cancelar", 
                    command=self.janela_material.destroy, style='Medium.TButton').pack(side='right', padx=5)
            
            # Inicializar
            self.carregar_opcoes_combos()
            self.atualizar_resumo()
            
            print("✅ Interface de materiais criada com sucesso!")
            
        except Exception as e:
            print(f"❌ Erro ao criar interface: {e}")
            import traceback
            print(f"Traceback: {traceback.format_exc()}")
            custom_messagebox("error", "Erro", f"Erro ao criar interface: {str(e)}")

    def toggle_dados_fornecedor(self):
        """Habilita/desabilita campos do fornecedor"""
        try:
            estado = 'normal' if self.tem_dados_compra.get() else 'disabled'
            
            for campo, widget in self.campos_fornecedor_fixo.items():
                widget.config(state=estado)
            
            print(f"Campos do fornecedor {'habilitados' if estado == 'normal' else 'desabilitados'}")
            
        except Exception as e:
            print(f"❌ Erro ao alternar campos do fornecedor: {e}")

    def toggle_campos_compra_material(self):
        """Método para compatibilidade com código antigo"""
        # Este método pode ser chamado pelo código antigo, redirecionamos para o novo
        if hasattr(self, 'toggle_dados_fornecedor'):
            return self.toggle_dados_fornecedor()
        else:
            print("⚠️ Método toggle_campos_compra_material chamado mas não implementado neste contexto")

    def carregar_opcoes_combos(self):
        """Carrega opções para os comboboxes"""
        try:
            # Definir opções básicas
            self.opcoes_categoria = ['ACABAMENTOS', 'ESTRUTURAL', 'INSTALAÇÕES', 'PAISAGISMO', 'OUTROS']
            self.opcoes_ambiente = ['SALA', 'QUARTO', 'COZINHA', 'BANHEIRO', 'ÁREA EXTERNA', 'GERAL']
            self.opcoes_unidade = ['UN', 'M²', 'M³', 'M', 'KG', 'L', 'PC', 'CX', 'SC', 'PAR']
            self.opcoes_status = ['PENDENTE', 'INSTALADO', 'AGUARDANDO', 'DEFEITO']
            
            # Tentar carregar do gerenciador se existir
            if hasattr(self, 'gerenciador_materiais') and hasattr(self.gerenciador_materiais, 'parametros'):
                parametros = self.gerenciador_materiais.parametros
                
                if 'categorias_materiais' in parametros:
                    self.opcoes_categoria = list(parametros['categorias_materiais'].keys())
                
                if 'ambientes' in parametros:
                    self.opcoes_ambiente = parametros['ambientes']
                
                if 'unidades' in parametros:
                    self.opcoes_unidade = parametros['unidades']
            
            print("✅ Opções dos combos carregadas")
            
        except Exception as e:
            print(f"❌ Erro ao carregar opções: {e}")

    def abrir_formulario_material(self, material_para_editar=None):
        """Abre formulário completo para adicionar/editar material"""
        try:
            # Janela do formulário
            self.janela_form_material = tk.Toplevel(self.janela_material)
            self.janela_form_material.title("Novo Material" if material_para_editar is None else "Editar Material")
            self.janela_form_material.geometry("800x700")
            self.janela_form_material.transient(self.janela_material)
            self.janela_form_material.grab_set()
            
            # Frame principal com scroll
            canvas = tk.Canvas(self.janela_form_material)
            scrollbar = ttk.Scrollbar(self.janela_form_material, orient="vertical", command=canvas.yview)
            ...
            # ========== DADOS BÁSICOS ==========
            frame_basicos = ttk.LabelFrame(scrollable_frame, text="📋 Dados Básicos", padding=15)
            frame_basicos.pack(fill='x', padx=10, pady=5)
            
            self.campos_form_material = {}
            
            # Linha 1: Categoria e Subcategoria
            tk.Label(frame_basicos, text="Categoria:*", font=('Arial', 9, 'bold')).grid(row=0, column=0, sticky='w', pady=5)
            self.campos_form_material['categoria'] = ttk.Combobox(frame_basicos, width=20, state='readonly', values=self.opcoes_categoria)
            self.campos_form_material['categoria'].grid(row=0, column=1, sticky='ew', padx=5, pady=5)
            
            tk.Label(frame_basicos, text="Subcategoria:", font=('Arial', 9)).grid(row=0, column=2, sticky='w', pady=5)
            self.campos_form_material['subcategoria'] = ttk.Combobox(frame_basicos, width=20, state='readonly')
            self.campos_form_material['subcategoria'].grid(row=0, column=3, sticky='ew', padx=5, pady=5)
            
            # Linha 2: Código e Descrição
            tk.Label(frame_basicos, text="Código:", font=('Arial', 9)).grid(row=1, column=0, sticky='w', pady=5)
            self.campos_form_material['codigo'] = tk.Entry(frame_basicos, width=15)
            self.campos_form_material['codigo'].grid(row=1, column=1, sticky='w', padx=5, pady=5)
            
            tk.Label(frame_basicos, text="Descrição:*", font=('Arial', 9, 'bold')).grid(row=1, column=2, sticky='w', pady=5)
            self.campos_form_material['descricao'] = tk.Entry(frame_basicos, width=40)
            self.campos_form_material['descricao'].grid(row=1, column=3, sticky='ew', padx=5, pady=5)
            
            # Linha 3: Marca e Modelo
            tk.Label(frame_basicos, text="Marca:", font=('Arial', 9)).grid(row=2, column=0, sticky='w', pady=5)
            self.campos_form_material['marca'] = tk.Entry(frame_basicos, width=20)
            self.campos_form_material['marca'].grid(row=2, column=1, sticky='ew', padx=5, pady=5)
            
            tk.Label(frame_basicos, text="Modelo:", font=('Arial', 9)).grid(row=2, column=2, sticky='w', pady=5)
            self.campos_form_material['modelo'] = tk.Entry(frame_basicos, width=20)
            self.campos_form_material['modelo'].grid(row=2, column=3, sticky='ew', padx=5, pady=5)
            
            # Linha 4: Cor e Dimensões
            tk.Label(frame_basicos, text="Cor/Acabamento:", font=('Arial', 9)).grid(row=3, column=0, sticky='w', pady=5)
            self.campos_form_material['cor'] = tk.Entry(frame_basicos, width=20)
            self.campos_form_material['cor'].grid(row=3, column=1, sticky='ew', padx=5, pady=5)
            
            tk.Label(frame_basicos, text="Dimensões:", font=('Arial', 9)).grid(row=3, column=2, sticky='w', pady=5)
            self.campos_form_material['dimensoes'] = tk.Entry(frame_basicos, width=20)
            self.campos_form_material['dimensoes'].grid(row=3, column=3, sticky='ew', padx=5, pady=5)
            
            # ========== QUANTIDADE E VALORES ==========
            frame_valores = ttk.LabelFrame(scrollable_frame, text="💰 Quantidade e Valores", padding=15)
            frame_valores.pack(fill='x', padx=10, pady=5)
            
            # Linha 1: Quantidade e Unidade
            tk.Label(frame_valores, text="Quantidade:*", font=('Arial', 9, 'bold')).grid(row=0, column=0, sticky='w', pady=5)
            self.campos_form_material['quantidade'] = tk.Entry(frame_valores, width=10)
            self.campos_form_material['quantidade'].insert(0, '1')
            self.campos_form_material['quantidade'].grid(row=0, column=1, sticky='w', padx=5, pady=5)
            
            tk.Label(frame_valores, text="Unidade:*", font=('Arial', 9, 'bold')).grid(row=0, column=2, sticky='w', pady=5)
            self.campos_form_material['unidade'] = ttk.Combobox(frame_valores, width=10, state='readonly', values=self.opcoes_unidade)
            self.campos_form_material['unidade'].set('UN')
            self.campos_form_material['unidade'].grid(row=0, column=3, sticky='w', padx=5, pady=5)
            
            # Linha 2: Valores
            tk.Label(frame_valores, text="Valor Unitário:", font=('Arial', 9)).grid(row=1, column=0, sticky='w', pady=5)
            self.campos_form_material['valor_unit'] = tk.Entry(frame_valores, width=15)
            self.campos_form_material['valor_unit'].insert(0, '0,00')
            self.campos_form_material['valor_unit'].grid(row=1, column=1, sticky='ew', padx=5, pady=5)
            
            tk.Label(frame_valores, text="Valor Total:", font=('Arial', 9)).grid(row=1, column=2, sticky='w', pady=5)
            self.campos_form_material['valor_total'] = tk.Entry(frame_valores, width=15, state='readonly')
            self.campos_form_material['valor_total'].grid(row=1, column=3, sticky='ew', padx=5, pady=5)
            
            # Bind para calcular valor total automaticamente
            def calcular_valor_total(event=None):
                try:
                    qtd_str = self.campos_form_material['quantidade'].get().replace(',', '.')
                    vlr_str = self.campos_form_material['valor_unit'].get().replace(',', '.')
                    
                    if qtd_str and vlr_str:
                        qtd = float(qtd_str)
                        vlr = float(vlr_str)
                        total = qtd * vlr
                        
                        self.campos_form_material['valor_total'].config(state='normal')
                        self.campos_form_material['valor_total'].delete(0, tk.END)
                        self.campos_form_material['valor_total'].insert(0, f"{total:.2f}".replace('.', ','))
                        self.campos_form_material['valor_total'].config(state='readonly')
                except:
                    pass
            
            self.campos_form_material['quantidade'].bind('<KeyRelease>', calcular_valor_total)
            self.campos_form_material['valor_unit'].bind('<KeyRelease>', calcular_valor_total)
            
            # ========== LOCALIZAÇÃO ==========
            frame_localizacao = ttk.LabelFrame(scrollable_frame, text="📍 Localização e Status", padding=15)
            frame_localizacao.pack(fill='x', padx=10, pady=5)
            
            # Linha 1: Ambiente e Status
            tk.Label(frame_localizacao, text="Ambiente:", font=('Arial', 9)).grid(row=0, column=0, sticky='w', pady=5)
            self.campos_form_material['ambiente'] = ttk.Combobox(frame_localizacao, width=20, state='readonly', values=self.opcoes_ambiente)
            self.campos_form_material['ambiente'].grid(row=0, column=1, sticky='ew', padx=5, pady=5)
            
            tk.Label(frame_localizacao, text="Status:", font=('Arial', 9)).grid(row=0, column=2, sticky='w', pady=5)
            self.campos_form_material['status'] = ttk.Combobox(frame_localizacao, width=15, state='readonly', values=self.opcoes_status)
            self.campos_form_material['status'].set('PENDENTE')
            self.campos_form_material['status'].grid(row=0, column=3, sticky='ew', padx=5, pady=5)
            
            # Linha 2: Instalação
            tk.Label(frame_localizacao, text="Data Instalação:", font=('Arial', 9)).grid(row=1, column=0, sticky='w', pady=5)
            self.campos_form_material['data_instalacao'] = tk.Entry(frame_localizacao, width=15)
            self.campos_form_material['data_instalacao'].grid(row=1, column=1, sticky='w', padx=5, pady=5)
            
            tk.Label(frame_localizacao, text="Instalador:", font=('Arial', 9)).grid(row=1, column=2, sticky='w', pady=5)
            self.campos_form_material['instalador'] = tk.Entry(frame_localizacao, width=30)
            self.campos_form_material['instalador'].grid(row=1, column=3, sticky='ew', padx=5, pady=5)
            
            # Linha 3: Garantia
            tk.Label(frame_localizacao, text="Garantia (meses):", font=('Arial', 9)).grid(row=2, column=0, sticky='w', pady=5)
            self.campos_form_material['garantia'] = tk.Entry(frame_localizacao, width=10)
            self.campos_form_material['garantia'].insert(0, '12')
            self.campos_form_material['garantia'].grid(row=2, column=1, sticky='w', padx=5, pady=5)
            
            # ========== OBSERVAÇÕES ==========
            frame_obs = ttk.LabelFrame(scrollable_frame, text="📝 Observações", padding=15)
            frame_obs.pack(fill='both', expand=True, padx=10, pady=5)
            
            self.campos_form_material['observacoes'] = tk.Text(frame_obs, height=4, wrap='word')
            self.campos_form_material['observacoes'].pack(fill='both', expand=True, padx=5, pady=5)
            
            # ========== BOTÕES ==========
            frame_botoes_form = ttk.Frame(scrollable_frame)
            frame_botoes_form.pack(fill='x', padx=10, pady=10)
            
            # Variável para controlar se estamos editando
            self.material_editando = material_para_editar
            
            texto_botao = "💾 Salvar Material" if material_para_editar is None else "✅ Atualizar Material"
            ttk.Button(frame_botoes_form, text=texto_botao, 
                    command=self.salvar_material_formulario, style='Medium.TButton').pack(side='left', padx=5)
            
            ttk.Button(frame_botoes_form, text="🧹 Limpar", 
                    command=self.limpar_formulario_material, style='Medium.TButton').pack(side='left', padx=5)
            
            ttk.Button(frame_botoes_form, text="❌ Cancelar", 
                    command=self.janela_form_material.destroy, style='Medium.TButton').pack(side='right', padx=5)
            
            # Configurar grids
            for frame in [frame_basicos, frame_valores, frame_localizacao]:
            ...
            # Se estamos editando, preencher campos
            if material_para_editar is not None:
                self.preencher_formulario_edicao(material_para_editar)
            
            # Bind para subcategorias
            self.campos_form_material['categoria'].bind('<<ComboboxSelected>>', self.atualizar_subcategorias_form)
            
            print("✅ Formulário de material aberto")
            
        except Exception as e:
            print(f"❌ Erro ao abrir formulário: {e}")
            import traceback
            print(f"Traceback: {traceback.format_exc()}")
            custom_messagebox("error", "Erro", f"Erro ao abrir formulário: {str(e)}")

    def preencher_formulario_edicao(self, material):
        """Preenche formulário com dados do material para edição"""
        try:
            # Preencher todos os campos com dados do material
            campos_mapeamento = {
                'categoria': 'categoria',
            ...
            for campo_form, campo_material in campos_mapeamento.items():
                if campo_form in self.campos_form_material and campo_material in material:
                    valor = material[campo_material]
                    widget = self.campos_form_material[campo_form]
            ...
            # Observações
            if 'observacoes' in material:
                self.campos_form_material['observacoes'].delete('1.0', tk.END)
                self.campos_form_material['observacoes'].insert('1.0', material['observacoes'])
            
            print("✅ Formulário preenchido para edição")
            
        except Exception as e:
            print(f"❌ Erro ao preencher formulário: {e}")

    def atualizar_subcategorias_form(self, event=None):
        """Atualiza subcategorias no formulário"""
        try:
            categoria = self.campos_form_material['categoria'].get()
            
            if hasattr(self, 'gerenciador_materiais') and categoria:
                parametros = getattr(self.gerenciador_materiais, 'parametros', {})
                categorias = parametros.get('categorias_materiais', {})
                
                if categoria in categorias:
                    subcategorias = categorias[categoria].get('subcategorias', [])
                    self.campos_form_material['subcategoria']['values'] = subcategorias
                    self.campos_form_material['subcategoria'].set('')
                else:
                    self.campos_form_material['subcategoria']['values'] = []
                    self.campos_form_material['subcategoria'].set('')
            else:
                self.campos_form_material['subcategoria']['values'] = []
                self.campos_form_material['subcategoria'].set('')
    ...
    def salvar_material_formulario(self):
        """Salva material do formulário na lista em memória"""
        try:
            # Validar campos obrigatórios
            if not self.campos_form_material['categoria'].get().strip():
                custom_messagebox("error", "Erro", "Categoria é obrigatória!")
                return
            
            if not self.campos_form_material['descricao'].get().strip():
                custom_messagebox("error", "Erro", "Descrição é obrigatória!")
                return
            
            # Coletar dados do formulário
            material = {
                'categoria': self.campos_form_material['categoria'].get().strip(),
                'subcategoria': self.campos_form_material['subcategoria'].get().strip(),
                'codigo': self.campos_form_material['codigo'].get().strip(),
                'descricao': self.campos_form_material['descricao'].get().strip(),
                'marca': self.campos_form_material['marca'].get().strip(),
                'modelo': self.campos_form_material['modelo'].get().strip(),
                'cor': self.campos_form_material['cor'].get().strip(),
                'dimensoes': self.campos_form_material['dimensoes'].get().strip(),
                'quantidade': self.campos_form_material['quantidade'].get().strip(),
                'unidade': self.campos_form_material['unidade'].get().strip(),
                'valor_unit': self.campos_form_material['valor_unit'].get().strip(),
                'valor_total': self.campos_form_material['valor_total'].get().strip(),
                'ambiente': self.campos_form_material['ambiente'].get().strip(),
                'status': self.campos_form_material['status'].get().strip(),
                'data_instalacao': self.campos_form_material['data_instalacao'].get().strip(),
                'instalador': self.campos_form_material['instalador'].get().strip(),
                'garantia': self.campos_form_material['garantia'].get().strip(),
                'observacoes': self.campos_form_material['observacoes'].get('1.0', tk.END).strip()
            }
            
            # Validar quantidade como número
            try:
                float(material['quantidade'].replace(',', '.'))
            except ValueError:
                custom_messagebox("error", "Erro", "Quantidade deve ser um número válido!")
                return
            
            # Validar valor unitário se preenchido
            if material['valor_unit'] and material['valor_unit'] != '0,00':
                try:
                    float(material['valor_unit'].replace(',', '.'))
                except ValueError:
                    custom_messagebox("error", "Erro", "Valor unitário deve ser um número válido!")
                    return
            
            if self.material_editando is not None:
                # Modo edição - atualizar material existente
                indice = self.material_editando
                self.lista_materiais[indice] = material
                print(f"✅ Material {indice + 1} atualizado")
                custom_messagebox("info", "Sucesso", "Material atualizado com sucesso!")
            else:
                # Modo novo - adicionar à lista
                self.lista_materiais.append(material)
                print(f"➕ Material adicionado à lista (total: {len(self.lista_materiais)})")
                custom_messagebox("info", "Sucesso", "Material adicionado à lista!")
            ...
            # Fechar formulário
            self.janela_form_material.destroy()
            
        except Exception as e:
            print(f"❌ Erro ao salvar material do formulário: {e}")
            import traceback
            print(f"Traceback: {traceback.format_exc()}")
            custom_messagebox("error", "Erro", f"Erro ao salvar material: {str(e)}")

    def limpar_formulario_material(self):
        """Limpa todos os campos do formulário"""
        try:
            for campo, widget in self.campos_form_material.items():
                if campo in ['quantidade', 'valor_unit', 'garantia']:
                    # Campos com valores padrão
                    widget.delete(0, tk.END)
                    if campo == 'quantidade':
                        widget.insert(0, '1')
                    elif campo == 'valor_unit':
                        widget.insert(0, '0,00')
                    elif campo == 'garantia':
                        widget.insert(0, '12')
                elif campo in ['unidade', 'status']:
                    # Comboboxes com valores padrão
                    if campo == 'unidade':
                        widget.set('UN')
                    elif campo == 'status':
                        widget.set('PENDENTE')
    ...
    def atualizar_lista_materiais(self):
        """Atualiza a visualização da lista de materiais"""
        try:
            # Limpar tree
            ...
            # Adicionar materiais
            for i, material in enumerate(self.lista_materiais):
                # Preparar marca/modelo
                marca_modelo = f"{material['marca']} {material['modelo']}".strip()
            ...
            print(f"📋 Lista atualizada: {len(self.lista_materiais)} materiais")
            
        except Exception as e:
            print(f"❌ Erro ao atualizar lista: {e}")

    def atualizar_resumo(self):
        """Atualiza o resumo da lista"""
        try:
            total_materiais = len(self.lista_materiais)
            
            if total_materiais == 0:
                self.label_resumo.config(text="Nenhum material adicionado")
            else:
                ...
                for material in self.lista_materiais:
                    try:
                        if material['valor_total']:
                            valor = float(material['valor_total'].replace(',', '.'))
                            valor_total += valor
                    except:
                        pass
                
                texto_resumo = f"{total_materiais} material(s) adicionado(s)"
                if valor_total > 0:
                    texto_resumo += f" | Valor total: R$ {valor_total:,.2f}".replace('.', ',')
                
                self.label_resumo.config(text=texto_resumo)
            
        except Exception as e:
            print(f"❌ Erro ao atualizar resumo: {e}")

    def editar_material_selecionado_nova_interface(self):
        """Edita o material selecionado na nova interface"""
        try:
            selecao = self.tree_materiais.selection()
            if not selecao:
                custom_messagebox("warning", "Aviso", "Selecione um material para editar!")
                return
            
            # Obter índice do material selecionado
            item = selecao[0]
            children = self.tree_materiais.get_children()
            indice = children.index(item)
            
            # Abrir formulário para edição
            self.abrir_formulario_material(indice)
            
        except Exception as e:
            print(f"❌ Erro ao editar material: {e}")

    def editar_material_selecionado(self):
        """Método mantido para compatibilidade com código antigo"""
        return self.editar_material_selecionado_nova_interface()

    def duplicar_material_selecionado(self):
        """Duplica o material selecionado"""
        try:
            selecao = self.tree_materiais.selection()
            if not selecao:
                custom_messagebox("warning", "Aviso", "Selecione um material para duplicar!")
                return
            ...
            material_original = self.lista_materiais[indice].copy()
            
            # Adicionar cópia à lista
            self.lista_materiais.append(material_original)
            
            # Atualizar visualização
            self.atualizar_lista_materiais()
            self.atualizar_resumo()
            
            print("📋 Material duplicado")
            custom_messagebox("info", "Sucesso", "Material duplicado com sucesso!")
            
        except Exception as e:
            print(f"❌ Erro ao duplicar material: {e}")

    def remover_material_selecionado(self):
        """Remove o material selecionado da lista"""
        try:
            selecao = self.tree_materiais.selection()
            if not selecao:
                custom_messagebox("warning", "Aviso", "Selecione um material para remover!")
                return
            
            # Confirmar remoção
            from tkinter import messagebox
            if not messagebox.askyesno("Confirmar", "Deseja remover o material selecionado?"):
                return
            
            # Obter índice e remover
            item = selecao[0]
            children = self.tree_materiais.get_children()
            indice = children.index(item)
            
            material_removido = self.lista_materiais.pop(indice)
            
            # Atualizar visualização
            self.atualizar_lista_materiais()
            self.atualizar_resumo()
            
            print(f"🗑️ Material removido: {material_removido['descricao']}")
            custom_messagebox("info", "Sucesso", "Material removido da lista!")
            
        except Exception as e:
            print(f"❌ Erro ao remover material: {e}")

    def limpar_lista_materiais(self):
        """Limpa toda a lista de materiais"""
        try:
            if not self.lista_materiais:
                custom_messagebox("info", "Informação", "A lista já está vazia!")
                return
            
            # Confirmar limpeza
            from tkinter import messagebox
            if not messagebox.askyesno("Confirmar", f"Deseja remover todos os {len(self.lista_materiais)} materiais da lista?"):
                return
            
            # Limpar lista
            self.lista_materiais.clear()
            
            # Atualizar visualização
            self.atualizar_lista_materiais()
            self.atualizar_resumo()
            
            print("🧹 Lista de materiais limpa")
            custom_messagebox("info", "Sucesso", "Lista de materiais limpa!")
            
        except Exception as e:
            print(f"❌ Erro ao limpar lista: {e}")

    def salvar_todos_materiais(self):
        """Salva todos os materiais da lista no arquivo Excel"""
        import pandas as pd
        from pathlib import Path
        from datetime import datetime
        
        try:
            print("💾 Iniciando salvamento de múltiplos materiais...")
            
            # Verificar se há materiais na lista
            if not self.lista_materiais:
                custom_messagebox("warning", "Aviso", "Adicione pelo menos um material à lista!")
                return
            
            # Preparar dados do fornecedor (comuns a todos)
            dados_fornecedor = {}
            if self.tem_dados_compra.get():
                dados_fornecedor = {
                    'Nome_Fornecedor': self.campos_fornecedor_fixo['fornecedor'].get().strip(),
                    'CNPJ_Fornecedor': self.campos_fornecedor_fixo['cnpj'].get().strip(),
                    'Data_Compra': self.campos_fornecedor_fixo['data_compra'].get().strip(),
                    'Numero_NF': self.campos_fornecedor_fixo['nf'].get().strip(),
                ...
                dados_fornecedor = {
                    ...
                    'Data_Compra': '',
                    'Numero_NF': '',
                    'Tem_Dados_Compra': False
                }
            
            # Carregar arquivo existente
            if hasattr(self, 'gerenciador_materiais') and hasattr(self.gerenciador_materiais, 'arquivo_materiais'):
                arquivo_materiais = self.gerenciador_materiais.arquivo_materiais
            else:
                base_path = getattr(self, 'BASE_PATH', Path.cwd())
                arquivo_materiais = base_path / 'materiais_obra.xlsx'
            
            try:
                df_existente = pd.read_excel(arquivo_materiais)
                ultimo_id = int(df_existente['ID'].max()) if len(df_existente) > 0 and 'ID' in df_existente.columns else 0
                print(f"📂 Arquivo carregado: {len(df_existente)} registros, último ID: {ultimo_id}")
            except FileNotFoundError:
            ...
            for material in self.lista_materiais:
                ...
                # Garantir que valores são strings para evitar erro do .strip()
                def safe_get(campo, padrao=''):
                    valor = material.get(campo, padrao)
                    return str(valor).strip() if valor is not None else padrao
                
                # Mapear dados do material
                dados_material = {
                    'ID': ultimo_id,
                    'Cliente': getattr(self, 'cliente_atual', 'SEM_CLIENTE'),
                    'Data_Cadastro': datetime.now().strftime('%d/%m/%Y'),
                    'Categoria': safe_get('categoria'),
                    'Subcategoria': safe_get('subcategoria'),
                    'Codigo_Produto': safe_get('codigo'),
                    'Descricao_Completa': safe_get('descricao'),
                    'Marca': safe_get('marca'),
                    'Modelo': safe_get('modelo'),
                    'Cor_Acabamento': safe_get('cor'),
                    'Dimensoes': safe_get('dimensoes'),
                    'Ambiente_Aplicacao': safe_get('ambiente'),
                    'Quantidade': safe_get('quantidade', '1'),
                    'Unidade': safe_get('unidade', 'UN'),
                    'Valor_Unitario': safe_get('valor_unit', '0'),
                    'Valor_Total': safe_get('valor_total', '0'),
                    'Data_Instalacao': safe_get('data_instalacao'),
                    'Instalador': safe_get('instalador'),
                    'Status_Instalacao': safe_get('status', 'PENDENTE'),
                    'Garantia_Meses': safe_get('garantia', '0'),
                    'Observacoes': safe_get('observacoes')
                }
                
                # Adicionar dados do fornecedor
                dados_material.update(dados_fornecedor)
                
                materiais_salvos.append(dados_material)
            
            # Criar DataFrame com novos materiais
            df_novos = pd.DataFrame(materiais_salvos)
            ...
            # Salvar arquivo
            df_final.to_excel(arquivo_materiais, index=False)
            
            print(f"✅ {len(materiais_salvos)} materiais salvos com sucesso!")
            
            # Calcular valor total para o resumo
            valor_total = 0
            for material in materiais_salvos:
                try:
                    if material['Valor_Total']:
                        valor = float(str(material['Valor_Total']).replace(',', '.'))
                        valor_total += valor
                except:
                    pass
            
            # Mensagem de sucesso
            mensagem = f"Salvamento concluído!\n\n{len(materiais_salvos)} materiais cadastrados\nIDs: {materiais_salvos[0]['ID']} a {materiais_salvos[-1]['ID']}"
            if valor_total > 0:
                mensagem += f"\nValor total: R$ {valor_total:,.2f}".replace('.', ',')
            
            custom_messagebox("info", "Sucesso", mensagem)
            
            # Perguntar se quer continuar ou fechar
            from tkinter import messagebox
            if messagebox.askyesno("Continuar", "Deseja cadastrar mais materiais?"):
                # Limpar apenas a lista, manter dados do fornecedor
                self.lista_materiais.clear()
                self.atualizar_lista_materiais()
                self.atualizar_resumo()
            else:
            ...
            import traceback
            print(f"Traceback: {traceback.format_exc()}")
            custom_messagebox("error", "Erro", f"Erro ao salvar materiais:\n{str(e)}")

    # MÉTODO PARA PREENCHER DADOS DA NF (chamado pelo checkbox)
    def cadastrar_material_vinculado_nf(self):
        """Cadastra material com dados da NF já preenchidos"""
        try:
            print("🔗 Abrindo cadastro de materiais vinculado à NF...")
            
            # Abrir janela de cadastro
            self.novo_material_direto()
            
            # Aguardar criação da janela e preencher dados
            self.janela_material.after(100, self.preencher_dados_nf_na_interface)
            
        except Exception as e:
            print(f"❌ Erro ao abrir materiais vinculados: {e}")
            custom_messagebox("error", "Erro", f"Erro ao abrir cadastro: {str(e)}")
    ...
    def preencher_dados_nf_na_interface(self):
        ...
        try:
            ...
            if hasattr(self, 'campos_fornecedor') and hasattr(self, 'campos_despesa'):
                ...
                print("⚠️ Campos da tela principal não encontrados")
                custom_messagebox("info", "Informação", 
                    "Interface de materiais aberta.\nPreencha os dados do fornecedor e adicione os materiais.")
    ...
    def carregar_opcoes_material(self):
        """Carrega opções nos comboboxes do material"""
        try:
            # Verificar se gerenciador existe
            if not hasattr(self, 'gerenciador_materiais'):
                # Criar opções básicas se não houver gerenciador
                categorias_basicas = ['ACABAMENTOS', 'ESTRUTURAL', 'INSTALAÇÕES', 'PAISAGISMO', 'OUTROS']
                ambientes_basicos = ['INSTALAÇÃO DA OBRA', 'SALA', 'QUARTO', 'COZINHA', 'BANHEIRO', 'ÁREA EXTERNA', 'GERAL']
                
                self.campos_material['categoria']['values'] = categorias_basicas
                self.campos_material['ambiente']['values'] = ambientes_basicos
                
            else:
                # Usar configurações do gerenciador
                if hasattr(self.gerenciador_materiais, 'parametros'):
                    categorias = list(self.gerenciador_materiais.parametros.get('categorias_materiais', {}).keys())
                    ambientes = self.gerenciador_materiais.parametros.get('ambientes', [])
                    
                    self.campos_material['categoria']['values'] = categorias if categorias else ['OUTROS']
                    self.campos_material['ambiente']['values'] = ambientes if ambientes else ['GERAL']
                
            # Bind para atualizar subcategorias quando categoria muda
            self.campos_material['categoria'].bind('<<ComboboxSelected>>', self.atualizar_subcategorias_material)
            
            print("✅ Opções dos comboboxes carregadas")
    ...
    def adicionar_secao_materiais_fornecedor(self):
        ...
        # Adicionar após o frame de taxas
        frame_materiais = ttk.LabelFrame(self.aba_fornecedor, text="Gestão de Materiais da Obra")
        frame_materiais.pack(fill='x', padx=10, pady=5)
        ...
        ttk.Button(
            frame_botoes_materiais,  # frame existente
            text="📄 Importar NF-e",
            command=self.criar_interface_importacao_nfe,
            style='Medium.TButton'
        ).pack(side='left', padx=5)

        # ttk.Button(
        #     frame_botoes_materiais,
        #     text="📊 Relatório de Materiais",
        #     command=self.relatorio_materiais,
        #     style='Medium.TButton'
        # ).pack(side='left', padx=5)

    def criar_aba_dados_basicos_material(self):
        """Cria aba de dados básicos do material"""
        
        frame_basicos = ttk.Frame(self.notebook_material)
        self.notebook_material.add(frame_basicos, text="Dados Básicos")
        
        # Frame principal com scroll
        canvas = tk.Canvas(frame_basicos)
        scrollbar = ttk.Scrollbar(frame_basicos, orient="vertical", command=canvas.yview)
        scrollable_frame = ttk.Frame(canvas)
        
        # Seção de identificação
        frame_id = ttk.LabelFrame(scrollable_frame, text="Identificação do Material", padding=15)
        ...
        self.campos_material['categoria'] = ttk.Combobox(frame_id, width=20, state='readonly')
        self.campos_material['categoria'].grid(row=0, column=1, sticky='ew', padx=5, pady=5)
        
        # Carregar categorias
        categorias = list(self.gerenciador_materiais.parametros['categorias_materiais'].keys())
        self.campos_material['categoria']['values'] = categorias
        self.campos_material['categoria'].bind('<<ComboboxSelected>>', self.atualizar_subcategorias_material)
        
        tk.Label(frame_id, text="Subcategoria:").grid(row=0, column=2, sticky='w', pady=5)
        self.campos_material['subcategoria'] = ttk.Combobox(frame_id, width=20, state='readonly')
        self.campos_material['subcategoria'].grid(row=0, column=3, sticky='ew', padx=5, pady=5)
        
        # Linha 2: Descrição e Código
        tk.Label(frame_id, text="Descrição:*", font=('Arial', 9, 'bold')).grid(row=1, column=0, sticky='w', pady=5)
        self.campos_material['descricao'] = tk.Entry(frame_id, width=50)
        self.campos_material['descricao'].grid(row=1, column=1, columnspan=2, sticky='ew', padx=5, pady=5)
        
        tk.Label(frame_id, text="Código:").grid(row=1, column=3, sticky='w', pady=5)
        self.campos_material['codigo'] = tk.Entry(frame_id, width=15)
        self.campos_material['codigo'].grid(row=1, column=4, sticky='ew', padx=5, pady=5)
        
        # Linha 3: Marca e Modelo
        tk.Label(frame_id, text="Marca:").grid(row=2, column=0, sticky='w', pady=5)
        self.campos_material['marca'] = tk.Entry(frame_id, width=20)
        self.campos_material['marca'].grid(row=2, column=1, sticky='ew', padx=5, pady=5)
        
        tk.Label(frame_id, text="Modelo:").grid(row=2, column=2, sticky='w', pady=5)
        self.campos_material['modelo'] = tk.Entry(frame_id, width=20)
        self.campos_material['modelo'].grid(row=2, column=3, sticky='ew', padx=5, pady=5)
        
        # Linha 4: Cor e Dimensões
        tk.Label(frame_id, text="Cor/Acabamento:").grid(row=3, column=0, sticky='w', pady=5)
        self.campos_material['cor'] = tk.Entry(frame_id, width=20)
        self.campos_material['cor'].grid(row=3, column=1, sticky='ew', padx=5, pady=5)
        
        tk.Label(frame_id, text="Dimensões:").grid(row=3, column=2, sticky='w', pady=5)
        self.campos_material['dimensoes'] = tk.Entry(frame_id, width=20)
        self.campos_material['dimensoes'].grid(row=3, column=3, sticky='ew', padx=5, pady=5)
    ...
    def criar_aba_fornecedor_material(self):
        """Cria aba de dados de fornecedor e compra"""
        
        frame_fornecedor = ttk.Frame(self.notebook_material)
        self.notebook_material.add(frame_fornecedor, text="Fornecedor (Opcional)")
        
        # Checkbox para indicar se tem dados de compra
        self.tem_dados_compra = tk.BooleanVar()
        checkbox_frame = ttk.Frame(frame_fornecedor)
        checkbox_frame.pack(fill='x', padx=10, pady=10)
        
        checkbox_compra = tk.Checkbutton(
            checkbox_frame, 
            text="Este material possui dados de compra/fornecedor", 
            variable=self.tem_dados_compra,
            command=self.toggle_campos_compra_material,
            font=('Arial', 10, 'bold')
        )
        checkbox_compra.pack(anchor='w', pady=5)
        
        # Frame para campos de compra
        self.frame_dados_compra = ttk.LabelFrame(frame_fornecedor, text="Dados da Compra", padding=15)
        self.frame_dados_compra.pack(fill='x', padx=10, pady=10)
        
        # Campos de fornecedor
        tk.Label(self.frame_dados_compra, text="Fornecedor:").grid(row=0, column=0, sticky='w', pady=5)
        self.campos_material['fornecedor'] = tk.Entry(self.frame_dados_compra, width=40, state='disabled')
        self.campos_material['fornecedor'].grid(row=0, column=1, columnspan=2, sticky='ew', padx=5, pady=5)
        
        tk.Label(self.frame_dados_compra, text="CNPJ/CPF:").grid(row=1, column=0, sticky='w', pady=5)
        self.campos_material['cnpj_fornecedor'] = tk.Entry(self.frame_dados_compra, width=20, state='disabled')
        self.campos_material['cnpj_fornecedor'].grid(row=1, column=1, sticky='ew', padx=5, pady=5)
        
        tk.Label(self.frame_dados_compra, text="Data Compra:").grid(row=1, column=2, sticky='w', pady=5)
        self.campos_material['data_compra'] = tk.Entry(self.frame_dados_compra, width=15, state='disabled')
        self.campos_material['data_compra'].grid(row=1, column=3, sticky='ew', padx=5, pady=5)
        
        # Valores e quantidades
        tk.Label(self.frame_dados_compra, text="Quantidade:").grid(row=2, column=0, sticky='w', pady=5)
        self.campos_material['quantidade'] = tk.Entry(self.frame_dados_compra, width=10, state='disabled')
        self.campos_material['quantidade'].grid(row=2, column=1, sticky='w', padx=5, pady=5)
        
        tk.Label(self.frame_dados_compra, text="Unidade:").grid(row=2, column=2, sticky='w', pady=5)
        self.campos_material['unidade'] = ttk.Combobox(self.frame_dados_compra, width=10, state='disabled')
        self.campos_material['unidade']['values'] = self.gerenciador_materiais.parametros['unidades']
        self.campos_material['unidade'].grid(row=2, column=3, sticky='w', padx=5, pady=5)
        
        tk.Label(self.frame_dados_compra, text="Valor Unit.:").grid(row=3, column=0, sticky='w', pady=5)
        self.campos_material['valor_unit'] = tk.Entry(self.frame_dados_compra, width=15, state='disabled')
        self.campos_material['valor_unit'].grid(row=3, column=1, sticky='ew', padx=5, pady=5)
        
        tk.Label(self.frame_dados_compra, text="Valor Total:").grid(row=3, column=2, sticky='w', pady=5)
        self.campos_material['valor_total'] = tk.Entry(self.frame_dados_compra, width=15, state='disabled')
        self.campos_material['valor_total'].grid(row=3, column=3, sticky='ew', padx=5, pady=5)
        
        # Nota Fiscal
        tk.Label(self.frame_dados_compra, text="Nº NF:").grid(row=4, column=0, sticky='w', pady=5)
        self.campos_material['nf'] = tk.Entry(self.frame_dados_compra, width=20, state='disabled')
        self.campos_material['nf'].grid(row=4, column=1, sticky='ew', padx=5, pady=5)
        
        # Configurar grid
        self.frame_dados_compra.columnconfigure(1, weight=1)
        self.frame_dados_compra.columnconfigure(3, weight=1)

    def criar_aba_instalacao_material(self):
        """Cria aba de instalação e localização"""
        
        frame_instalacao = ttk.Frame(self.notebook_material)
        self.notebook_material.add(frame_instalacao, text="Instalação & Localização")
        
        # Frame de localização
        frame_local = ttk.LabelFrame(frame_instalacao, text="Localização na Obra", padding=15)
        frame_local.pack(fill='x', padx=10, pady=10)
        
        tk.Label(frame_local, text="Ambiente:").grid(row=0, column=0, sticky='w', pady=5)
        self.campos_material['ambiente'] = ttk.Combobox(frame_local, width=20, state='readonly')
        self.campos_material['ambiente']['values'] = self.gerenciador_materiais.parametros['ambientes']
        self.campos_material['ambiente'].grid(row=0, column=1, sticky='ew', padx=5, pady=5)
        
        # Frame de instalação
        frame_inst = ttk.LabelFrame(frame_instalacao, text="Dados da Instalação", padding=15)
        frame_inst.pack(fill='x', padx=10, pady=10)
        
        tk.Label(frame_inst, text="Data Instalação:").grid(row=0, column=0, sticky='w', pady=5)
        self.campos_material['data_instalacao'] = tk.Entry(frame_inst, width=15)
        self.campos_material['data_instalacao'].grid(row=0, column=1, sticky='w', padx=5, pady=5)
        
        tk.Label(frame_inst, text="Instalador:").grid(row=0, column=2, sticky='w', pady=5)
        self.campos_material['instalador'] = tk.Entry(frame_inst, width=30)
        self.campos_material['instalador'].grid(row=0, column=3, sticky='ew', padx=5, pady=5)
        
        tk.Label(frame_inst, text="Status:").grid(row=1, column=0, sticky='w', pady=5)
        self.campos_material['status'] = ttk.Combobox(frame_inst, width=15, state='readonly')
        self.campos_material['status']['values'] = self.gerenciador_materiais.parametros['status_instalacao']
        self.campos_material['status'].set('PENDENTE')
        self.campos_material['status'].grid(row=1, column=1, sticky='w', padx=5, pady=5)
        
        tk.Label(frame_inst, text="Garantia (meses):").grid(row=1, column=2, sticky='w', pady=5)
        self.campos_material['garantia'] = tk.Entry(frame_inst, width=10)
        self.campos_material['garantia'].grid(row=1, column=3, sticky='w', padx=5, pady=5)
        
        # Observações
        frame_obs = ttk.LabelFrame(frame_instalacao, text="Observações e Especificações Técnicas", padding=15)
        frame_obs.pack(fill='both', expand=True, padx=10, pady=10)
        
        self.campos_material['observacoes'] = tk.Text(frame_obs, height=6, wrap='word')
        self.campos_material['observacoes'].pack(fill='both', expand=True, padx=5, pady=5)
        
        # Configurar grids
        frame_local.columnconfigure(1, weight=1)
        frame_inst.columnconfigure(3, weight=1)

    def atualizar_subcategorias_material(self, event=None):
        """Atualiza subcategorias baseado na categoria selecionada"""
        try:
            categoria = self.campos_material['categoria'].get()
            ...
            if hasattr(self, 'gerenciador_materiais') and categoria:
                ...
                if categoria in categorias:
                    subcategorias = categorias[categoria].get('subcategorias', [])
                    self.campos_material['subcategoria']['values'] = subcategorias
                    self.campos_material['subcategoria'].set('')
                else:
                    self.campos_material['subcategoria']['values'] = []
                    self.campos_material['subcategoria'].set('')
            else:
                # Sem gerenciador, usar subcategorias básicas
                self.campos_material['subcategoria']['values'] = []
                self.campos_material['subcategoria'].set('')
    ...
    def toggle_campos_compra_material(self):
        """Habilita/desabilita campos de compra baseado no checkbox"""
        try:
            estado = 'normal' if self.tem_dados_compra.get() else 'disabled'
            
            # Lista de campos que devem ser habilitados/desabilitados
            campos_compra = [
                'fornecedor', 'cnpj_fornecedor', 'data_compra', 'quantidade', 
                'valor_unit', 'valor_total', 'nf'
            ]
            
            for campo in campos_compra:
                if campo in self.campos_material:
                    if hasattr(self.campos_material[campo], 'config'):
                        self.campos_material[campo].config(state=estado)
            
            # Combobox tem tratamento diferente
            if 'unidade' in self.campos_material:
                estado_combo = 'readonly' if estado == 'normal' else 'disabled'
                self.campos_material['unidade'].config(state=estado_combo)
            
            print(f"Campos de compra {'habilitados' if estado == 'normal' else 'desabilitados'}")
            
        except Exception as e:
            print(f"❌ Erro ao alternar campos: {e}")

    def criar_botoes_material(self):
        """Cria botões de ação na janela de material"""
        
        frame_botoes = ttk.Frame(self.janela_material)
        ...
        ttk.Button(
            frame_botoes, 
            text="📷 Adicionar Foto", 
            command=self.adicionar_foto_material,
            style='Medium.TButton'
        ).pack(side='left', padx=5)
        ...
        ttk.Button(
            frame_botoes, 
            text="📄 Anexar Manual", 
            command=self.anexar_manual_material,
            style='Medium.TButton'
        ).pack(side='left', padx=5)
        ...
        ttk.Button(
            frame_botoes, 
            text="❌ Cancelar", 
            command=self.janela_material.destroy,
            style='Medium.TButton'
        ).pack(side='right', padx=5)

    def salvar_material(self):
        """Salva material na planilha Excel - versão com imports corretos"""
        
        try:
            print("💾 Iniciando salvamento do material...")
            
            # Validar campos obrigatórios
            if not self.campos_material['categoria'].get().strip():
                custom_messagebox("error", "Erro", "Categoria é obrigatória!")
                return
            
            if not self.campos_material['descricao'].get().strip():
                custom_messagebox("error", "Erro", "Descrição é obrigatória!")
                return
            ...
            dados_material = {
                'Cliente': getattr(self, 'cliente_atual', 'SEM_CLIENTE'),
                'Data_Cadastro': datetime.now().strftime('%d/%m/%Y'),
                'Categoria': self.campos_material['categoria'].get().strip(),
                'Subcategoria': self.campos_material['subcategoria'].get().strip(),
                'Codigo_Produto': self.campos_material['codigo'].get().strip(),
                'Descricao_Completa': self.campos_material['descricao'].get().strip(),
                'Marca': self.campos_material['marca'].get().strip(),
                'Modelo': self.campos_material['modelo'].get().strip(),
                'Cor_Acabamento': self.campos_material['cor'].get().strip(),
                'Dimensoes': self.campos_material['dimensoes'].get().strip(),
                'Ambiente_Aplicacao': self.campos_material['ambiente'].get().strip(),
                'Data_Instalacao': self.campos_material['data_instalacao'].get().strip(),
                'Instalador': self.campos_material['instalador'].get().strip(),
                'Status_Instalacao': self.campos_material['status'].get() or 'PENDENTE',
                'Garantia_Meses': self.campos_material['garantia'].get().strip() or '0',
                'Observacoes': self.campos_material['observacoes'].get('1.0', tk.END).strip(),
                'Tem_Dados_Compra': self.tem_dados_compra.get()
            }
            
            print(f"📋 Material: {dados_material['Descricao_Completa']}")
            
            # Adicionar dados de compra se existirem
            if self.tem_dados_compra.get():
                print("💰 Incluindo dados de compra...")
                dados_material.update({
                    'Nome_Fornecedor': self.campos_material['fornecedor'].get().strip(),
                    'CNPJ_Fornecedor': self.campos_material['cnpj_fornecedor'].get().strip(),
                    'Data_Compra': self.campos_material['data_compra'].get().strip(),
                    'Quantidade': self.campos_material['quantidade'].get().strip(),
                    'Unidade': self.campos_material['unidade'].get().strip(),
                    'Valor_Unitario': self.campos_material['valor_unit'].get().strip(),
                    'Valor_Total': self.campos_material['valor_total'].get().strip(),
                    'Numero_NF': self.campos_material['nf'].get().strip()
                })
            else:
                print("ℹ️ Sem dados de compra")
                # Preencher campos vazios para manter estrutura
                dados_material.update({
                    'Nome_Fornecedor': '',
                    'CNPJ_Fornecedor': '',
                    'Data_Compra': '',
            ...
            if hasattr(self, 'gerenciador_materiais') and hasattr(self.gerenciador_materiais, 'arquivo_materiais'):
                arquivo_materiais = self.gerenciador_materiais.arquivo_materiais
                print(f"📂 Usando arquivo do gerenciador: {arquivo_materiais}")
            else:
                base_path = getattr(self, 'BASE_PATH', Path.cwd())
                arquivo_materiais = base_path / 'materiais_obra.xlsx'
                print(f"📂 Usando arquivo padrão: {arquivo_materiais}")
            
            # Carregar arquivo existente ou criar novo
            try:
                df_existente = pd.read_excel(arquivo_materiais)
                print(f"📊 Arquivo carregado: {len(df_existente)} registros existentes")
            except FileNotFoundError:
            ...
            dados_material['ID'] = ultimo_id + 1
            print(f"🆔 Novo ID gerado: {dados_material['ID']}")
            
            # Criar nova linha e adicionar ao DataFrame
            ...
            # Salvar arquivo
            df_atualizado.to_excel(arquivo_materiais, index=False)
            
            print(f"✅ Material salvo com sucesso! ID: {dados_material['ID']}")
            
            custom_messagebox("info", "Sucesso", 
                f"Material cadastrado com sucesso!\n\nID: {dados_material['ID']}\nDescrição: {dados_material['Descricao_Completa']}")
            
            # Perguntar se quer cadastrar outro material
            resposta = messagebox.askyesno("Novo Material", "Deseja cadastrar outro material?")
            ...
            custom_messagebox("error", "Erro", f"Erro ao salvar material:\n{str(e)}")

    def limpar_campos_material(self):
        """Limpa todos os campos do material"""
        try:
            for campo, widget in self.campos_material.items():
    ...
    def consultar_materiais(self):
        ...
        janela_consulta.title("Materiais Cadastrados")
        janela_consulta.geometry("1400x700")
        janela_consulta.grab_set()
        
        # Frame de filtros
        frame_filtros = ttk.LabelFrame(janela_consulta, text="Filtros")
        ...
        tk.Label(filtros_frame, text="Categoria:").pack(side='left', padx=5)
        filtro_categoria = ttk.Combobox(filtros_frame, width=15, state='readonly')
        categorias = ['TODAS'] + list(self.gerenciador_materiais.parametros['categorias_materiais'].keys())
        filtro_categoria['values'] = categorias
        filtro_categoria.set('TODAS')
        filtro_categoria.pack(side='left', padx=5)
        
        tk.Label(filtros_frame, text="Ambiente:").pack(side='left', padx=5)
        filtro_ambiente = ttk.Combobox(filtros_frame, width=15, state='readonly')
        ambientes = ['TODOS'] + self.gerenciador_materiais.parametros['ambientes']
        filtro_ambiente['values'] = ambientes
        filtro_ambiente.set('TODOS')
        filtro_ambiente.pack(side='left', padx=5)
        
        tk.Label(filtros_frame, text="Status:").pack(side='left', padx=5)
        filtro_status = ttk.Combobox(filtros_frame, width=15, state='readonly')
        status = ['TODOS'] + self.gerenciador_materiais.parametros['status_instalacao']
        filtro_status['values'] = status
        filtro_status.set('TODOS')
        filtro_status.pack(side='left', padx=5)
        
        tk.Label(filtros_frame, text="Fonte:").pack(side='left', padx=5)
        filtro_fonte = ttk.Combobox(filtros_frame, width=12, state='readonly')
        filtro_fonte['values'] = ['TODOS', 'COM NF', 'SEM NF']
        filtro_fonte.set('TODOS')
        filtro_fonte.pack(side='left', padx=5)
        
        # Botões de filtro
        ttk.Button(filtros_frame, text="🔍 Filtrar", 
                command=lambda: self.aplicar_filtros_materiais(
                    tree_materiais, filtro_categoria.get(), 
                    filtro_ambiente.get(), filtro_status.get(), filtro_fonte.get()
                )).pack(side='left', padx=10)
        
        ttk.Button(filtros_frame, text="🔄 Limpar", 
                command=lambda: self.limpar_filtros_materiais(
                    filtro_categoria, filtro_ambiente, filtro_status, filtro_fonte, tree_materiais
                )).pack(side='left', padx=5)
        
        # TreeView para materiais
        frame_tree = ttk.Frame(janela_consulta)
        ...
        # TreeView
        colunas = ('ID', 'Categoria', 'Descrição', 'Marca/Modelo', 'Ambiente', 
                'Status', 'Fonte', 'Data_Cadastro')
        tree_materiais = ttk.Treeview(frame_tree, columns=colunas, show='headings',
                                    yscrollcommand=scroll_y.set, xscrollcommand=scroll_x.set)
        
        # Configurar colunas
        tree_materiais.heading('ID', text='ID')
        tree_materiais.heading('Categoria', text='Categoria')
        tree_materiais.heading('Descrição', text='Descrição')
        tree_materiais.heading('Marca/Modelo', text='Marca/Modelo')
        tree_materiais.heading('Ambiente', text='Ambiente')
        tree_materiais.heading('Status', text='Status')
        tree_materiais.heading('Fonte', text='Fonte')
        tree_materiais.heading('Data_Cadastro', text='Data Cadastro')
        
        # Larguras das colunas
        tree_materiais.column('ID', width=50)
        tree_materiais.column('Categoria', width=120)
        tree_materiais.column('Descrição', width=300)
        tree_materiais.column('Marca/Modelo', width=200)
        tree_materiais.column('Ambiente', width=150)
        tree_materiais.column('Status', width=120)
        tree_materiais.column('Fonte', width=80)
        tree_materiais.column('Data_Cadastro', width=100)
        
        tree_materiais.pack(side='left', fill='both', expand=True)
        
        # Configurar scrollbars
        scroll_y.config(command=tree_materiais.yview)
        scroll_x.config(command=tree_materiais.xview)
        
        # Carregar dados iniciais
        self.carregar_materiais_tree(tree_materiais)
        
        # Frame de botões de ação
        frame_acoes = ttk.Frame(janela_consulta)
        frame_acoes.pack(fill='x', padx=10, pady=5)
        
        ttk.Button(frame_acoes, text="✏️ Editar", 
                command=lambda: self.editar_material_selecionado(tree_materiais)).pack(side='left', padx=5)
        ttk.Button(frame_acoes, text="🗑️ Excluir", 
                command=lambda: self.excluir_material_selecionado(tree_materiais)).pack(side='left', padx=5)
        ttk.Button(frame_acoes, text="📄 Manual", 
                command=self.gerar_manual_proprietario).pack(side='left', padx=5)
        ttk.Button(frame_acoes, text="📊 Exportar", 
                command=lambda: self.exportar_materiais(tree_materiais)).pack(side='left', padx=5)
        
        # Bind para duplo clique
        tree_materiais.bind('<Double-1>', lambda e: self.ver_detalhes_material(tree_materiais))

    def carregar_materiais_tree(self, tree):
        """Carrega materiais na TreeView"""
        try:
            # Limpar tree
            ...
            if self.gerenciador_materiais.arquivo_materiais.exists():
                df = pd.read_excel(self.gerenciador_materiais.arquivo_materiais)
                
                # Filtrar por cliente atual
                df_cliente = df[df['Cliente'] == self.cliente_atual] if 'Cliente' in df.columns else df
                
                for _, row in df_cliente.iterrows():
                    # Determinar fonte
                    fonte = 'COM NF' if pd.notna(row.get('Numero_NF', '')) and row.get('Numero_NF', '') != '' else 'SEM NF'
                    
                    # Combinar marca e modelo
                    marca_modelo = f"{row.get('Marca', '')} {row.get('Modelo', '')}".strip()
                    
                    tree.insert('', 'end', values=(
                        row.get('ID', ''),
                        row.get('Categoria', ''),
                        row.get('Descricao_Completa', ''),
                        marca_modelo,
                        row.get('Ambiente_Aplicacao', ''),
                        row.get('Status_Instalacao', ''),
                        fonte,
                        row.get('Data_Cadastro', '')
                    ))
                    
        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao carregar materiais: {str(e)}")

    def aplicar_filtros_materiais(self, tree, categoria, ambiente, status, fonte):
        """Aplica filtros na visualização de materiais"""
        try:
            # Limpar tree
            for item in tree.get_children():
                tree.delete(item)
            
            if not self.gerenciador_materiais.arquivo_materiais.exists():
                return
                
            df = pd.read_excel(self.gerenciador_materiais.arquivo_materiais)
            
            # Filtrar por cliente
            df_filtrado = df[df['Cliente'] == self.cliente_atual] if 'Cliente' in df.columns else df
            ...
            if ambiente != 'TODOS':
                df_filtrado = df_filtrado[df_filtrado['Ambiente_Aplicacao'] == ambiente]
            
            if status != 'TODOS':
                df_filtrado = df_filtrado[df_filtrado['Status_Instalacao'] == status]
            
            if fonte != 'TODOS':
                if fonte == 'COM NF':
                    df_filtrado = df_filtrado[pd.notna(df_filtrado['Numero_NF']) & (df_filtrado['Numero_NF'] != '')]
                else:  # SEM NF
                    df_filtrado = df_filtrado[pd.isna(df_filtrado['Numero_NF']) | (df_filtrado['Numero_NF'] == '')]
            
            # Preencher tree com dados filtrados
            for _, row in df_filtrado.iterrows():
                fonte_row = 'COM NF' if pd.notna(row.get('Numero_NF', '')) and row.get('Numero_NF', '') != '' else 'SEM NF'
                marca_modelo = f"{row.get('Marca', '')} {row.get('Modelo', '')}".strip()
    ...
    def editar_material_selecionado(self, tree):
        ...
        # Por enquanto, apenas mostrar informação
        item = tree.item(selection[0])
        material_id = item['values'][0]
        messagebox.showinfo("Editar", f"Função de edição será implementada.\nMaterial ID: {material_id}")

    def excluir_material_selecionado(self, tree):
        ...
        if messagebox.askyesno("Confirmar", f"Deseja excluir o material?\n\nID: {material_id}\nDescrição: {descricao}"):
            # Por enquanto, apenas remover da tree (implementar exclusão da planilha depois)
            tree.delete(selection[0])
            messagebox.showinfo("Sucesso", "Material removido da visualização.\n(Implementar exclusão permanente)")

    def ver_detalhes_material(self, tree):
        """Mostra detalhes do material selecionado"""
        selection = tree.selection()
        if not selection:
            return
        ...
        # Buscar dados completos na planilha
        try:
            df_cliente = self.gerenciador_materiais.carregar_materiais_cliente(self.cliente_atual)
            material = df_cliente[df_cliente['ID'] == int(material_id)]
            ...
            if len(material) > 0:
                ...
                janela_detalhes.title(f"Detalhes do Material - ID {material_id}")
                janela_detalhes.geometry("600x500")
                janela_detalhes.grab_set()
                
                # Criar texto com detalhes
                texto = tk.Text(janela_detalhes, wrap='word', padx=10, pady=10)
                texto.pack(fill='both', expand=True, padx=10, pady=10)
                
                # Preencher detalhes
                detalhes = f"""DETALHES DO MATERIAL - ID {material_id}
    {'='*50}

    DADOS BÁSICOS:
    • Categoria: {material_data.get('Categoria', 'N/A')}
    • Subcategoria: {material_data.get('Subcategoria', 'N/A')}
    • Descrição: {material_data.get('Descricao_Completa', 'N/A')}
    • Código: {material_data.get('Codigo_Produto', 'N/A')}
    • Marca: {material_data.get('Marca', 'N/A')}
    • Modelo: {material_data.get('Modelo', 'N/A')}
    • Cor/Acabamento: {material_data.get('Cor_Acabamento', 'N/A')}
    • Dimensões: {material_data.get('Dimensoes', 'N/A')}

    LOCALIZAÇÃO:
    • Ambiente: {material_data.get('Ambiente_Aplicacao', 'N/A')}
    • Status: {material_data.get('Status_Instalacao', 'N/A')}
    • Instalador: {material_data.get('Instalador', 'N/A')}
    • Data Instalação: {material_data.get('Data_Instalacao', 'N/A')}
    • Garantia: {material_data.get('Garantia_Meses', '0')} meses

    DADOS DE COMPRA:
    """
                
                if material_data.get('Tem_Dados_Compra'):
                    detalhes += f"""• Fornecedor: {material_data.get('Nome_Fornecedor', 'N/A')}
    • CNPJ: {material_data.get('CNPJ_Fornecedor', 'N/A')}
    • Data Compra: {material_data.get('Data_Compra', 'N/A')}
    • Quantidade: {material_data.get('Quantidade', 'N/A')} {material_data.get('Unidade', '')}
    • Valor Unitário: R$ {material_data.get('Valor_Unitario', '0')}
    • Valor Total: R$ {material_data.get('Valor_Total', '0')}
    • Nota Fiscal: {material_data.get('Numero_NF', 'N/A')}
    """
                else:
                    detalhes += "• Sem dados de compra cadastrados\n"
                
                if material_data.get('Observacoes'):
                    detalhes += f"\nOBSERVAÇÕES:\n{material_data.get('Observacoes', '')}"
                
                detalhes += f"\n\nDATA DE CADASTRO: {material_data.get('Data_Cadastro', 'N/A')}"
    ...
    def integrar_consultor_scraping(self):
        """Modifica o método consultar_nfe_publica para usar scraping"""
        
        # Substituir o método na classe IntegradorNFe
        def consultar_nfe_publica_real(self, chave_acesso):
            """Consulta NF-e via scraping (método modificado)"""
            try:
                print(f"🔍 Consultando NF-e via scraping: {chave_acesso}")
                
                # Inicializar consultor de scraping
                if not hasattr(self, 'consultor_scraping'):
                    self.consultor_scraping = ConsultorNFeScraping()
                
                # Fazer consulta
                dados_extraidos = self.consultor_scraping.consultar_nfe(chave_acesso)
                
                if not dados_extraidos:
                    raise Exception("Não foi possível extrair dados da NF-e")
                
                return dados_extraidos
                
            except Exception as e:
                print(f"❌ Erro na consulta NF-e: {e}")
                # Fallback para simulação se scraping falhar
                print("🔄 Tentando método de simulação...")
                return self.simular_extracao_nfe(chave_acesso, self.extrair_info_chave(chave_acesso))
        
    def criar_interface_importacao_nfe(self):
        """Cria interface para importação de NF-e"""
        
        self.janela_nfe = tk.Toplevel(self.root)
        self.janela_nfe.title("Importação de NF-e")
        self.janela_nfe.geometry("800x600")
        self.janela_nfe.grab_set()
        
        # Inicializar integrador se não existir
        if not hasattr(self, 'integrador_nfe'):
            API_KEY_NFE_IO = "sua_api_key_aqui"  # Obtida no painel da NFe.io
            self.integrador_nfe = IntegradorNFeIO(self, API_KEY_NFE_IO)
        
        # Frame principal
        frame_principal = ttk.Frame(self.janela_nfe)
        frame_principal.pack(fill='both', expand=True, padx=10, pady=10)
        
        # Seção 1: Input da chave
        frame_chave = ttk.LabelFrame(frame_principal, text="Dados da NF-e", padding=10)
        frame_chave.pack(fill='x', pady=5)
        
        tk.Label(frame_chave, text="Chave de Acesso (44 dígitos):*", 
                font=('Arial', 9, 'bold')).pack(anchor='w')
        
        self.entry_chave = tk.Entry(frame_chave, width=50, font=('Courier', 10))
        self.entry_chave.pack(fill='x', pady=5)
        
        # Botão consultar
        ttk.Button(frame_chave, text="🔍 Consultar NF-e", 
                command=self.consultar_e_processar_nfe).pack(pady=5)
        
        # Seção 2: Dados da NF (inicialmente oculto)
        self.frame_dados_nfe = ttk.LabelFrame(frame_principal, text="Dados Encontrados", padding=10)
        
        # Seção 3: Opções de importação (inicialmente oculto)  
        self.frame_opcoes = ttk.LabelFrame(frame_principal, text="Opções de Importação", padding=10)
        
        # Seção 4: Lista de produtos (inicialmente oculto)
        self.frame_produtos = ttk.LabelFrame(frame_principal, text="Produtos/Serviços", padding=10)
        
        # Botões finais
        frame_botoes = ttk.Frame(frame_principal)
        frame_botoes.pack(fill='x', pady=10, side='bottom')
        
        self.btn_importar = ttk.Button(frame_botoes, text="📥 Importar Selecionados", 
                                    command=self.executar_importacao, state='disabled')
        self.btn_importar.pack(side='left', padx=5)
        
        ttk.Button(frame_botoes, text="❌ Cancelar", 
                command=self.janela_nfe.destroy).pack(side='right', padx=5)

    def consultar_e_processar_nfe(self):
        """Consulta NF-e e processa dados"""
        try:
            chave = self.entry_chave.get().strip()
            if not chave:
                messagebox.showerror("Erro", "Informe a chave de acesso!")
                return
            ...
            # Consultar NF-e
            dados_nfe = self.integrador_nfe.consultar_nfe_publica(chave)
            
            if dados_nfe:
                self.dados_nfe_atual = dados_nfe
                self.exibir_dados_nfe(dados_nfe)
                self.criar_opcoes_importacao()
                self.exibir_produtos_nfe(dados_nfe.get('produtos', []))
                self.btn_importar.config(state='normal')
            
        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao consultar NF-e:\n{str(e)}")
        finally:
            self.entry_chave.config(state='normal')

    def exibir_dados_nfe(self, dados):
        """Exibe dados básicos da NF-e"""
        
        self.frame_dados_nfe.pack(fill='x', pady=5)
        
        # Limpar frame
        for widget in self.frame_dados_nfe.winfo_children():
            widget.destroy()
        
        # Criar grid de informações
        info_frame = ttk.Frame(self.frame_dados_nfe)
        info_frame.pack(fill='x')
        
        # Dados básicos
        informacoes = [
            ("Número NF:", dados.get('numero_nf', '')),
            ("Data Emissão:", dados.get('data_emissao', '')),
            ("CNPJ Emitente:", dados.get('cnpj_emitente', '')),
            ("Razão Social:", dados.get('razao_social_emitente', '')),
            ("Valor Total:", f"R$ {dados.get('valor_total', 0):,.2f}"),
            ("Qtd Produtos:", len(dados.get('produtos', [])))
        ]
    ...
    def criar_opcoes_importacao(self):
        ...
        tk.Label(self.frame_opcoes, text="Selecione o que deseja importar:", 
                font=('Arial', 10, 'bold')).pack(anchor='w', pady=5)
        
        # Checkboxes
        cb_frame = ttk.Frame(self.frame_opcoes)
        cb_frame.pack(fill='x', pady=5)
        
        cb_financeiro = tk.Checkbutton(
            cb_frame, 
            text="💰 Lançamento Financeiro (entrada de dados)", 
        ...
        cb_materiais = tk.Checkbutton(
            cb_frame, 
            text="📦 Materiais da Obra (produtos como materiais)", 
        ...
        info_text = tk.Text(self.frame_opcoes, height=3, wrap='word', 
                        bg='#f0f0f0', relief='flat')
        info_text.pack(fill='x', pady=5)
        
        info_content = """💡 Dica: Você pode importar apenas o financeiro, apenas os materiais, ou ambos.
    • Financeiro: Cria lançamento no sistema de entrada de dados
    • Materiais: Cadastra cada produto como material da obra para o manual do proprietário"""
        
        info_text.insert('1.0', info_content)
        info_text.config(state='disabled')

    def exibir_produtos_nfe(self, produtos):
        """Exibe lista de produtos da NF-e"""
        
        self.frame_produtos.pack(fill='both', expand=True, pady=5)
        ...
        if not produtos:
            tk.Label(self.frame_produtos, text="Nenhum produto encontrado na NF-e").pack()
            return
        
        # TreeView para produtos
        tree_frame = ttk.Frame(self.frame_produtos)
        ...
        # TreeView
        colunas = ('Código', 'Descrição', 'Qtd', 'Un', 'Vl Unit', 'Vl Total', 'Categoria')
        self.tree_produtos = ttk.Treeview(tree_frame, columns=colunas, show='headings',
        ...
        self.tree_produtos.heading('Un', text='Un')
        self.tree_produtos.heading('Vl Unit', text='Vl Unit')
        self.tree_produtos.heading('Vl Total', text='Vl Total')
        self.tree_produtos.heading('Categoria', text='Categoria Sugerida')
        
        # Larguras
        self.tree_produtos.column('Código', width=80)
        self.tree_produtos.column('Descrição', width=300)
        self.tree_produtos.column('Qtd', width=60)
        self.tree_produtos.column('Un', width=40)
        self.tree_produtos.column('Vl Unit', width=80)
        self.tree_produtos.column('Vl Total', width=80)
        self.tree_produtos.column('Categoria', width=120)
        
        self.tree_produtos.pack(side='left', fill='both', expand=True)
        scroll_y.config(command=self.tree_produtos.yview)
        ...
        # Checkboxes para selecionar produtos
        cb_frame = ttk.Frame(self.frame_produtos)
        cb_frame.pack(fill='x', pady=5)
        
        ttk.Button(cb_frame, text="✅ Selecionar Todos", 
                command=self.selecionar_todos_produtos).pack(side='left', padx=5)
        ttk.Button(cb_frame, text="❌ Desmarcar Todos", 
                command=self.desmarcar_todos_produtos).pack(side='left', padx=5)

    def selecionar_todos_produtos(self):
        """Seleciona todos os produtos"""
        for item in self.tree_produtos.get_children():
            self.tree_produtos.selection_add(item)

    def desmarcar_todos_produtos(self):
        """Desmarca todos os produtos"""
        self.tree_produtos.selection_remove(self.tree_produtos.selection())

    def executar_importacao(self):
        """Executa a importação conforme seleções"""
        try:
            if not self.dados_nfe_atual:
                messagebox.showerror("Erro", "Nenhuma NF-e carregada!")
                return
            
            # Verificar o que foi selecionado
            financeiro = self.importar_financeiro.get()
            materiais = self.importar_materiais.get()
            
            if not financeiro and not materiais:
                messagebox.showwarning("Aviso", "Selecione pelo menos uma opção de importação!")
                return
            
            resultados = []
            
            # 1. Importar dados financeiros
            if financeiro:
                resultado_financeiro = self.importar_dados_financeiros()
                resultados.append(f"✅ Lançamento financeiro: {resultado_financeiro}")
            
            # 2. Importar materiais
            if materiais:
                produtos_selecionados = self.obter_produtos_selecionados()
                if produtos_selecionados:
                    resultado_materiais = self.importar_dados_materiais(produtos_selecionados)
                    resultados.append(f"✅ Materiais importados: {resultado_materiais}")
                else:
                    resultados.append("⚠️ Nenhum produto selecionado para materiais")
            
            # Mostrar resultado
            messagebox.showinfo("Importação Concluída", "\n".join(resultados))
            
            # Fechar janela
            self.janela_nfe.destroy()
    ...
    def importar_dados_financeiros(self):
        ...
        try:
            ...
            # Preencher campos do sistema financeiro atual
            # (usando a estrutura do seu adicionar_dados existente)
            
            # Simular preenchimento dos campos
            dados_financeiros = {
                'data': dados.get('data_emissao', ''),
                'cnpj_cpf': dados.get('cnpj_emitente', ''),
                'nome': dados.get('razao_social_emitente', ''),
                'categoria': 'MAT',  # Ou determinar automaticamente
                'tp_desp': '3',  # Ou determinar automaticamente
                'referencia': 'MATERIAL OBRA',
                'nf': dados.get('numero_nf', ''),
                'vr_unit': f"{dados.get('valor_total', 0):.2f}",
                'dias': 1,
                'valor': f"{dados.get('valor_total', 0):.2f}",
                'dt_vencto': dados.get('data_emissao', ''),
                'dados_bancarios': '',
                'observacao': f"IMPORTADO NF-E {dados.get('chave_acesso', '')}",
                'forma_pagamento': ''  # Ou determinar
            }
            
            # Adicionar aos dados para incluir
            self.dados_para_incluir.append(dados_financeiros)
            
            return f"Valor R$ {dados.get('valor_total', 0):,.2f}"
            
        except Exception as e:
            raise Exception(f"Erro ao importar dados financeiros: {str(e)}")

    def obter_produtos_selecionados(self):
        """Obtém produtos selecionados na TreeView"""
        produtos_selecionados = []
        
        for item_id in self.tree_produtos.selection():
            # Encontrar produto original pelos dados da tree
            valores = self.tree_produtos.item(item_id, 'values')
            codigo = valores[0]
            
            # Buscar produto original
            for produto in self.dados_nfe_atual.get('produtos', []):
    ...
    def importar_dados_materiais(self, produtos):
        ...
        try:
            ...
            for produto in produtos:
                ...
                dados_material = {
                    ...
                    'Garantia_Meses': 0,
                    'Observacoes': f"Importado da NF-e {dados_nfe.get('numero_nf', '')}",
                    'Tem_Dados_Compra': True,
                ...
                # Salvar material
                self.gerenciador_materiais.salvar_material(dados_material)
                materiais_importados += 1
            
            return f"{materiais_importados} produto(s)"
            
        except Exception as e:
            raise Exception(f"Erro ao importar materiais: {str(e)}")
    
    def gerar_manual_proprietario(self):
        """Gera manual do proprietário - VERSÃO CORRIGIDA"""
        try:
            if not self.gerenciador_materiais.arquivo_materiais.exists():
                messagebox.showwarning("Aviso", "Nenhum material cadastrado ainda.")
                return
            
            df_cliente = self.gerenciador_materiais.carregar_materiais_cliente(self.cliente_atual)
            
            if len(df_cliente) == 0:
                messagebox.showwarning("Aviso", "Nenhum material cadastrado para este cliente.")
                return
            
            # Usar PASTA_CLIENTES que você já importa
            global PASTA_CLIENTES
            
            # Criar manual em texto
            data_hoje = datetime.now().strftime('%Y%m%d')
            nome_arquivo = f"Manual_Materiais_{self.cliente_atual}_{data_hoje}.txt"
            
            # Criar pasta do cliente se não existir
            pasta_cliente = PASTA_CLIENTES / self.cliente_atual
            pasta_cliente.mkdir(parents=True, exist_ok=True)
            
            caminho_arquivo = pasta_cliente / nome_arquivo
            
            # Gerar conteúdo
            with open(caminho_arquivo, 'w', encoding='utf-8') as f:
                f.write(f"MANUAL DE MATERIAIS - {self.cliente_atual}\n")
                f.write("=" * 60 + "\n\n")
                f.write(f"Gerado em: {datetime.now().strftime('%d/%m/%Y às %H:%M')}\n")
                f.write(f"Total de materiais: {len(df_cliente)}\n\n")
                
                # Agrupar por ambiente
                ambientes = df_cliente['Ambiente_Aplicacao'].dropna().unique()
                
                for ambiente in sorted(ambientes):
                    if pd.isna(ambiente) or ambiente == '':
                        continue
                        
                    materiais_ambiente = df_cliente[df_cliente['Ambiente_Aplicacao'] == ambiente]
                    
                    f.write(f"\n{ambiente}\n")
                    f.write("-" * len(ambiente) + "\n")
                    
                    for _, material in materiais_ambiente.iterrows():
                        f.write(f"\n• ID: {material.get('ID', 'N/A')}\n")
                        f.write(f"  Categoria: {material.get('Categoria', '')}\n")
                        f.write(f"  Descrição: {material.get('Descricao_Completa', '')}\n")
                        
                        if material.get('Marca'):
                            f.write(f"  Marca: {material['Marca']}")
                            if material.get('Modelo'):
                                f.write(f" - Modelo: {material['Modelo']}")
                            f.write("\n")
                        
                        if material.get('Cor_Acabamento'):
                            f.write(f"  Cor/Acabamento: {material['Cor_Acabamento']}\n")
                        
                        if material.get('Dimensoes'):
                            f.write(f"  Dimensões: {material['Dimensoes']}\n")
                        
                        f.write(f"  Status: {material.get('Status_Instalacao', 'N/A')}\n")
                        
                        if material.get('Instalador'):
                            f.write(f"  Instalador: {material['Instalador']}\n")
                        
                        if material.get('Garantia_Meses'):
                            f.write(f"  Garantia: {material['Garantia_Meses']} meses\n")
                        
                        if material.get('Observacoes'):
                            f.write(f"  Observações: {material['Observacoes']}\n")
                        
                        # Dados de compra se existirem
                        if material.get('Tem_Dados_Compra') and material.get('Nome_Fornecedor'):
                            f.write(f"  Fornecedor: {material['Nome_Fornecedor']}\n")
                            if material.get('Data_Compra'):
                                f.write(f"  Data da Compra: {material['Data_Compra']}\n")
                            if material.get('Numero_NF'):
                                f.write(f"  Nota Fiscal: {material['Numero_NF']}\n")
                        
                        f.write("\n" + "-" * 40 + "\n")
            
            messagebox.showinfo("Sucesso", f"Manual gerado com sucesso!\n\nArquivo: {caminho_arquivo}")
            
            # Perguntar se quer abrir
            if messagebox.askyesno("Abrir Manual", "Deseja abrir o manual gerado?"):
                import subprocess
                import platform
                
                if platform.system() == 'Windows':
                    subprocess.startfile(str(caminho_arquivo))
                elif platform.system() == 'Darwin':  # macOS
                    subprocess.call(['open', str(caminho_arquivo)])
                else:  # Linux
                    subprocess.call(['xdg-open', str(caminho_arquivo)])
                    
        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao gerar manual: {str(e)}")

    def criar_pdf_manual(self, ambientes_materiais):
        """Cria PDF do manual do proprietário"""
        try:
            ...
            data_hoje = datetime.now().strftime('%Y%m%d')
            nome_arquivo = f"Manual_Proprietario_{self.cliente_atual}_{data_hoje}.pdf"
            caminho_arquivo = self.config.PASTA_CLIENTES / self.cliente_atual / nome_arquivo
            
            # Criar pasta do cliente se não existir
            (self.config.PASTA_CLIENTES / self.cliente_atual).mkdir(parents=True, exist_ok=True)
            ...
            title = Paragraph(f"<b>MANUAL DO PROPRIETÁRIO</b><br/><i>{self.cliente_atual}</i>", title_style)
            story.append(title)
            ...
            data_para = Paragraph(f"Gerado em: {datetime.now().strftime('%d/%m/%Y às %H:%M')}", data_style)
            story.append(data_para)
            story.append(Spacer(1, 30))
            
            # Índice
            story.append(Paragraph("<b>ÍNDICE</b>", styles['Heading2']))
            for ambiente in sorted(ambientes_materiais.keys()):
                story.append(Paragraph(f"• {ambiente}", styles['Normal']))
            story.append(Spacer(1, 20))
            story.append(PageBreak())
            
            # Conteúdo por ambiente
            for ambiente in sorted(ambientes_materiais.keys()):
                materiais = ambientes_materiais[ambiente]
                ...
                story.append(Paragraph(f"<b>{ambiente}</b>", env_style))
                
                # Criar tabela de materiais
                data = [['Item', 'Especificações', 'Marca/Modelo', 'Garantia', 'Instalador']]
                
                for material in materiais:
                    categoria = material.get('Categoria', '')
                    subcategoria = material.get('Subcategoria', '')
                    item = f"{categoria}"
                    if subcategoria:
                        item += f"<br/><i>{subcategoria}</i>"
                    
                    especificacoes = []
                    if material.get('Descricao_Completa'):
                        especificacoes.append(material['Descricao_Completa'])
                    if material.get('Cor_Acabamento'):
                        especificacoes.append(f"Cor: {material['Cor_Acabamento']}")
                    if material.get('Dimensoes'):
                        especificacoes.append(f"Dim: {material['Dimensoes']}")
                    
                    marca_modelo = f"{material.get('Marca', '')} {material.get('Modelo', '')}".strip()
                    
                    garantia = f"{material.get('Garantia_Meses', 0)} meses" if material.get('Garantia_Meses') else 'N/A'
                    
                    instalador = material.get('Instalador', 'N/A')
                    
                    data.append([
                        Paragraph(item, styles['Normal']),
                        Paragraph('<br/>'.join(especificacoes), styles['Normal']),
                        Paragraph(marca_modelo, styles['Normal']),
                        Paragraph(garantia, styles['Normal']),
                        Paragraph(instalador, styles['Normal'])
                    ])
                
                # Criar e estilizar tabela
                table = Table(data, colWidths=[1.5*inch, 2.5*inch, 1.5*inch, 1*inch, 1.5*inch])
    ...
    def adicionar_foto_material(self):
        """Adiciona foto do material"""
        from tkinter import filedialog
        
        arquivo_foto = filedialog.askopenfilename(
            title="Selecionar Foto do Material",
            filetypes=[("Imagens", "*.jpg *.jpeg *.png *.bmp *.gif")]
        )
        
        if arquivo_foto:
            # Aqui você pode implementar a cópia da foto para uma pasta específica
            messagebox.showinfo("Foto", f"Foto selecionada: {arquivo_foto}")

    def anexar_manual_material(self):
        """Anexa manual do fabricante"""
        from tkinter import filedialog
        
        arquivo_manual = filedialog.askopenfilename(
            title="Selecionar Manual do Fabricante",
            filetypes=[("Documentos", "*.pdf *.doc *.docx"), ("Todos os arquivos", "*.*")]
        )
        
        if arquivo_manual:
            # Aqui você pode implementar a cópia do manual para uma pasta específica
            messagebox.showinfo("Manual", f"Manual selecionado: {arquivo_manual}")
...
class GerenciadorMateriais:
    ...
    def __init__(self, sistema_principal):
        ...
        # CORREÇÃO: Usar as variáveis globais que você já importa
        try:
            # Usar as constantes globais importadas no seu arquivo
            global BASE_PATH
            self.base_path = BASE_PATH
            print(f"✅ Usando BASE_PATH importado: {self.base_path}")
        except NameError:
            # Fallback - tentar importar diretamente
            try:
                from src.config.config import BASE_PATH
                self.base_path = BASE_PATH
                print(f"✅ BASE_PATH importado diretamente")
            except ImportError:
                try:
                    from config.config import BASE_PATH
                    self.base_path = BASE_PATH
                    print(f"✅ BASE_PATH importado via config")
                except ImportError:
                    # Último recurso
                    self.base_path = Path('C:/Users/Obras/sistema_gestao_testes/testes/Financeiro/Planilhas_Base')
    ...
    def criar_parametros_padrao(self):
        ...
        # O conteúdo já está no artifact anterior
        pass
    ...
    def inicializar_planilha_materiais(self):
        ...
        if not self.arquivo_materiais.exists():
            try:
                # Estrutura da planilha
                colunas = [
                    'ID', 'Cliente', 'Data_Cadastro', 'Data_Compra',
                    'CNPJ_Fornecedor', 'Nome_Fornecedor',
                    'Categoria', 'Subcategoria', 'Codigo_Produto',
                    'Descricao_Completa', 'Marca', 'Modelo',
                    'Cor_Acabamento', 'Dimensoes', 'Especificacoes_Tecnicas',
                    'Tem_Dados_Compra', 'Quantidade', 'Unidade',
                    'Valor_Unitario', 'Valor_Total', 'Numero_NF',
                    'Ambiente_Aplicacao', 'Data_Instalacao', 'Instalador',
                    'Status_Instalacao', 'Garantia_Meses',
    ...
    def carregar_materiais_cliente(self, cliente):
        ...
        try:
            ...
            return df[df['Cliente'] == cliente] if 'Cliente' in df.columns else df
    ...
    def salvar_material(self, dados_material):
        ...
        try:
            ...
            # Gerar ID único
            if len(df_existente) > 0:
                ultimo_id = df_existente['ID'].max() if 'ID' in df_existente.columns else 0
            else:
                ultimo_id = 0
            ...
            return dados_material['ID']
            
        except Exception as e:
            print(f"❌ Erro ao salvar material: {e}")
            raise e   

class IntegradorNFeIO:
    """Integra NFe.io com o sistema existente"""
    
    def __init__(self, sistema_principal, api_key_nfe_io):
        """
        Args:
            sistema_principal: Instância do sistema principal
            api_key_nfe_io (str): Chave da API NFe.io
        """
        self.sistema = sistema_principal
        self.dados_nfe_atual = None
        
        # Inicializar consultor NFe.io
        self.consultor_nfe_io = ConsultorNFeIO(api_key_nfe_io)
        
        # Testar conexão
        if not self.consultor_nfe_io.testar_conexao():
            print("⚠️ Problema na conexão com NFe.io - verifique a API Key")
    
    def consultar_nfe_publica(self, chave_acesso):
        """Método para substituir no sistema existente"""
        try:
            print(f"🔍 Consultando NF-e: {chave_acesso}")
            
            # Tentar consulta via NFe.io
            dados_nfe = self.consultor_nfe_io.consultar_nfe(chave_acesso)
            
            if dados_nfe:
                return dados_nfe
            else:
                raise Exception("NFe.io não retornou dados")
                
        except Exception as e:
            print(f"⚠️ NFe.io falhou: {e}")
            
            # Fallback para simulação
            print("🔄 Usando simulação como fallback...")
            return self.simular_extracao_nfe(chave_acesso, self.extrair_info_chave(chave_acesso))
    
    def simular_extracao_nfe(self, chave, info_chave):
        ...
        return {
            ...
            'razao_social_emitente': 'FORNECEDOR EXEMPLO LTDA',
            'valor_total': 1500.00,
            'produtos': [
                {
                    'codigo': 'PROD001',
                    'descricao': 'CERAMICA PORTINARI 60X60 POLIDO',
                    'quantidade': 25.0,
                    'unidade': 'M2',
                    'valor_unitario': 45.50,
                    'valor_total': 1137.50,
                    'categoria_sugerida': 'REVESTIMENTO',
                    'subcategoria_sugerida': 'CERAMICA'
                }
            ],
            'fonte_dados': 'Simulação (fallback)'
        }
    
    def validar_chave_acesso(self, chave):
        """Valida chave de acesso"""
        return self.consultor_nfe_io.validar_chave_acesso(chave)
    
    def extrair_info_chave(self, chave):
        """Extrai info da chave"""
        return self.consultor_nfe_io.extrair_info_chave(chave)


class ConsultorNFeIO:
    """Consulta NF-e usando a API da NFe.io"""
    
    def __init__(self, api_key):
        """
        Inicializa consultor NFe.io
        
        Args:
            api_key (str): Chave de API da NFe.io
        """
        ...
        self.session.headers.update({
            ...
            'User-Agent': 'Sistema-Entrada-Dados/1.0'
        })
        
        print("🔑 Consultor NFe.io inicializado")
    
    def consultar_nfe(self, chave_acesso):
        """
        Consulta NF-e via API da NFe.io
        
        Args:
            chave_acesso (str): Chave de acesso da NF-e (44 dígitos)
            
        Returns:
            dict: Dados da NF-e formatados
        """
        try:
            print(f"🔍 Consultando NF-e via NFe.io: {chave_acesso}")
            
            # Validar chave
            if not self.validar_chave_acesso(chave_acesso):
                raise ValueError("Chave de acesso inválida")
            
            # Fazer consulta na API
            dados_api = self.fazer_consulta_api(chave_acesso)
            
            if dados_api:
                # Processar e formatar dados
                dados_formatados = self.processar_dados_nfe_io(dados_api, chave_acesso)
                
                print(f"✅ Dados obtidos via NFe.io!")
                print(f"📄 {dados_formatados.get('razao_social_emitente', 'N/A')}")
                print(f"💰 R$ {dados_formatados.get('valor_total', 0):,.2f}")
                print(f"📦 {len(dados_formatados.get('produtos', []))} produto(s)")
            ...
            print(f"❌ Erro consulta NFe.io: {e}")
            raise e
    
    def fazer_consulta_api(self, chave_acesso):
        """Faz a consulta na API da NFe.io"""
        try:
            # URL da API de consulta (v2)
            url = f"{self.base_url}/v2/invoices/nfe/{chave_acesso}"
            ...
            if "Erro HTTP" in str(e) or "Timeout" in str(e) or "conexão" in str(e):
                raise e
            else:
                raise Exception(f"Erro na requisição: {e}")
    
    def processar_dados_nfe_io(self, dados_api, chave_acesso):
        """Processa dados da API NFe.io para formato padrão"""
        try:
            # Estrutura base
            dados_processados = {
                ...
                'valor_total': 0.0,
                'produtos': [],
                'fonte_dados': 'NFe.io API',
                'dados_brutos': dados_api  # Para debug
            }
            
            # A estrutura da resposta da NFe.io pode variar
            # Vamos tentar diferentes caminhos para extrair os dados
            
            # Caminho 1: Dados diretos na raiz
            if 'nfe' in dados_api:
                nfe_data = dados_api['nfe']
            elif 'invoice' in dados_api:
                nfe_data = dados_api['invoice']
            else:
                nfe_data = dados_api
            
            # Extrair número da NF-e
            numero = self.extrair_campo(nfe_data, ['numero', 'number', 'nNF', 'numeroNota'])
            if numero:
                dados_processados['numero_nf'] = str(numero)
            
            # Extrair data de emissão
            data_emissao = self.extrair_campo(nfe_data, ['dataEmissao', 'dhEmi', 'data', 'issueDate'])
            if data_emissao:
                dados_processados['data_emissao'] = self.formatar_data(data_emissao)
            
            # Extrair dados do emitente
            emitente = self.extrair_campo(nfe_data, ['emitente', 'issuer', 'emit'])
            if emitente:
                # CNPJ
                cnpj = self.extrair_campo(emitente, ['cnpj', 'CNPJ', 'federalTaxNumber'])
                if cnpj:
                    dados_processados['cnpj_emitente'] = str(cnpj)
                
                # Razão Social
                razao_social = self.extrair_campo(emitente, ['razaoSocial', 'xNome', 'name', 'companyName'])
                if razao_social:
                    dados_processados['razao_social_emitente'] = str(razao_social)
            
            # Extrair valor total
            valor_total = self.extrair_campo(nfe_data, ['valorTotal', 'vNF', 'totalValue', 'total'])
            if valor_total:
                dados_processados['valor_total'] = float(valor_total)
            
            # Extrair produtos/itens
            produtos = self.extrair_produtos_nfe_io(nfe_data)
            dados_processados['produtos'] = produtos
            
            # Se não conseguiu extrair dados essenciais, tentar pela chave
            if not dados_processados['numero_nf']:
                info_chave = self.extrair_info_chave(chave_acesso)
                dados_processados['numero_nf'] = info_chave['numero']
                dados_processados['cnpj_emitente'] = info_chave['cnpj_emitente']
            
            return dados_processados
            
        except Exception as e:
            raise Exception(f"Erro ao processar dados da NFe.io: {e}")
    
    def extrair_campo(self, data, campos_possiveis):
        """Extrai campo tentando diferentes nomes"""
    ...
    def extrair_produtos_nfe_io(self, nfe_data):
        """Extrai produtos da resposta da NFe.io"""
        produtos = []
        
        try:
            # Procurar lista de itens/produtos
            itens_paths = ['itens', 'items', 'produtos', 'det', 'lineItems']
            itens = None
            
            for path in itens_paths:
                if path in nfe_data and isinstance(nfe_data[path], list):
                    itens = nfe_data[path]
                    break
            
            if not itens:
                print("⚠️ Nenhum produto encontrado na resposta")
                return produtos
            ...
            for item in itens:
                ...
                # Extrair dados do produto (pode estar em 'prod' ou direto no item)
                prod_data = item.get('prod', item)
                
                # Código do produto
                codigo = self.extrair_campo(prod_data, ['cProd', 'codigo', 'code', 'productCode'])
                if codigo:
                    produto['codigo'] = str(codigo)
                
                # Descrição
                descricao = self.extrair_campo(prod_data, ['xProd', 'descricao', 'description', 'productName'])
                if descricao:
                    produto['descricao'] = str(descricao)
                
                # Quantidade
                quantidade = self.extrair_campo(prod_data, ['qCom', 'quantidade', 'quantity'])
                if quantidade:
                    produto['quantidade'] = float(quantidade)
                
                # Unidade
                unidade = self.extrair_campo(prod_data, ['uCom', 'unidade', 'unit'])
                if unidade:
                    produto['unidade'] = str(unidade)
                
                # Valor unitário
                valor_unit = self.extrair_campo(prod_data, ['vUnCom', 'valorUnitario', 'unitPrice'])
                if valor_unit:
                    produto['valor_unitario'] = float(valor_unit)
                
                # Valor total do item
                valor_total = self.extrair_campo(prod_data, ['vProd', 'valorTotal', 'totalPrice'])
                if valor_total:
                    produto['valor_total'] = float(valor_total)
                
                # Calcular valor total se não tiver
                if produto['valor_total'] == 0 and produto['quantidade'] > 0 and produto['valor_unitario'] > 0:
                ...
                # Adicionar se tiver descrição
                if produto['descricao']:
                    produtos.append(produto)
            
            print(f"📦 {len(produtos)} produto(s) extraído(s)")
    ...
    def classificar_produto(self, descricao):
        ...
        classificacoes = {
            'REVESTIMENTO': ['CERAMICA', 'PORCELANATO', 'AZULEJO', 'PASTILHA', 'REVESTIMENTO', 'PISO'],
            'TINTAS': ['TINTA', 'VERNIZ', 'ESMALTE', 'PRIMER', 'SELADOR'],
            'ELETRICO': ['FIO', 'CABO', 'TOMADA', 'INTERRUPTOR', 'LAMPADA', 'LED', 'ELETRICO'],
            'HIDRAULICO': ['TUBO', 'CONEXAO', 'REGISTRO', 'TORNEIRA', 'VALVULA', 'HIDRAULICO'],
            'ESTRUTURAL': ['CIMENTO', 'CONCRETO', 'FERRO', 'AÇO', 'TIJOLO', 'BLOCO', 'VIGA'],
            'ACABAMENTO': ['RODAPE', 'MOLDURA', 'REJUNTE', 'MASSA', 'ARGAMASSA', 'GESSO'],
            'ESQUADRIAS': ['PORTA', 'JANELA', 'FECHADURA', 'DOBRADIÇA', 'VIDRO', 'MARCO'],
            'FERRAGENS': ['PARAFUSO', 'PREGO', 'BUCHA', 'CHAVE', 'FECHADURA']
        }
        
        for categoria, palavras_chave in classificacoes.items():
            if any(palavra in descricao_upper for palavra in palavras_chave):
                return categoria
    ...
    def extrair_info_chave(self, chave):
        ...
        try:
            ...
            return {'uf': '00', 'ano_mes': '0000', 'cnpj_emitente': '00000000000000', 'numero': '000000000'}
    
    def testar_conexao(self):
        """Testa se a API Key está funcionando"""
        try:
            print("🧪 Testando conexão com NFe.io...")
            
            # Fazer uma requisição simples para testar
            url = f"{self.base_url}/v1/companies/current"
            response = self.session.get(url, timeout=10)
            
            if response.status_code == 200:
                print("✅ Conexão com NFe.io OK!")
                return True
            elif response.status_code == 401:
