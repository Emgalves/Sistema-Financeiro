...
class EditorLancamento:
    ...
    def atualizar_dados_bancarios(self, event=None):
        """Atualiza dados bancários baseado na forma de pagamento"""
        forma_pagamento = self.forma_pagamento.get()
        fornecedor = self.buscar_fornecedor_completo(self.dados['cnpj_cpf'])
        
        if fornecedor:
            if forma_pagamento == "PIX" and fornecedor['chave_pix']:
                dados_bancarios = f"PIX: {fornecedor['chave_pix']}"
            else:
                # Construir dados para TED
                partes_dados = []
                if fornecedor['banco']: partes_dados.append(fornecedor['banco'])
                if fornecedor['op']: partes_dados.append(fornecedor['op'])
                if fornecedor['agencia']: partes_dados.append(fornecedor['agencia'])
                if fornecedor['conta']: partes_dados.append(fornecedor['conta'])
            ...
            self.dados_bancarios.insert(0, dados_bancarios)
...
class SistemaEntradaDados:
    ...
    def setup_aba_dados(self):
        ...
        ttk.Label(frame_fornecedor, text="Dados Bancários:").grid(row=len(campos) + 1, column=0, padx=5, pady=2, sticky='e')
        entry = tk.Entry(frame_fornecedor, width=40, state='readonly')
        entry.grid(row=len(campos) + 1, column=1, padx=5, pady=2, sticky='ew')
    ...
    def setup_formulario_fornecedor(self, modo_edicao=False):
        ...
        carregar_configuracoes()  # Garante que as configurações estão atualizadas
    
        # No frame de dados bancários, alterar o campo de banco para Combobox
...
##        def validar_categoria(event=None):
##            categoria = self.campos_form['categoria'].get()
...
##                )
##                self.campos_form['categoria'].set('')
##                return False
##            return True
##
##        self.campos_form['categoria'].bind('<<ComboboxSelected>>', validar_categoria)
        ...
        try:
            ...
            if dados['tp_desp'] == '1' and dados['referencia'] == 'TRANSPORTE':
                try:
                    ...
                    from configuracoes_sistema import GerenciadorConfiguracoes
                    config = GerenciadorConfiguracoes.get_configuracoes()
                    
                    if config and 'cafe' in config:
                        vr_unit_cafe = config['cafe']['valor_atual']
                    else:
            ...
            for item in self.tree_adm.get_children():
                ...
                if valores[2] == 'Fixo' and 'entrada' in (tags or ()):
                    ...
                    try:
                        ...
                        # Usar as novas funções de formatação
                        valor = formatar_valor_excel(valores[3])
                        cell_vr_unit = ws_dados.cell(row=proxima_linha_dados, column=7, value=valor)
                        aplicar_formatacao_celula(cell_vr_unit) # Valor da entrada
                         
                        ws_dados.cell(row=proxima_linha_dados, column=8, value=1)

                        cell_valor = ws_dados.cell(row=proxima_linha_dados, column=9, value=valor)
