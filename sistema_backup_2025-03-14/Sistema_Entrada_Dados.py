    ...
    def atualizar_dados_bancarios(self, event=None):
        ...
        try:
            ...
            if forma_pagamento == "PIX" and fornecedor_completo['chave_pix']:
                ...
                # Construir dados para TED incluindo CNPJ/CPF
                dados_ted = []
                ...
                if forma_pagamento == "TED" and fornecedor_completo['cnpj_cpf']: 
                    dados_ted.append(str(fornecedor_completo['cnpj_cpf']))
                
                dados_bancarios = ' - '.join(filter(None, dados_ted))
            
            if not dados_bancarios or dados_bancarios.strip() in ['', ' - ']:
...
class SistemaEntradaDados:
    ...
    def atualizar_dados_bancarios(self, event=None):
        ...
        if forma_pagamento == "PIX" and fornecedor_completo['chave_pix']:
            ...
            if fornecedor_completo['agencia']: dados_ted.append(str(fornecedor_completo['agencia']))
            if fornecedor_completo['conta']: dados_ted.append(str(fornecedor_completo['conta']))
            if fornecedor_completo['cnpj_cpf']: dados_ted.append(str(fornecedor_completo['cnpj_cpf']))
...
class GestaoContratos:
    ...
    def criar_novo_contrato(self, janela_principal):
        """Abre janela para criar novo contrato com suporte a percentual"""
        janela = tk.Toplevel(self.parent)
        janela.title(f"Novo Contrato - {self.cliente_atual}")
        janela.geometry("650x550")  # Aumentar um pouco o tamanho da janela
        ...
        observacoes = ttk.Entry(frame_contrato, width=25)  # Aumentei a largura
        observacoes.grid(row=3, column=1, padx=5, pady=5, sticky='ew')
        
        # Configure column weights to allow expansion
        frame_contrato.columnconfigure(1, weight=1)
    ...
    def adicionar_administrador(self, tree):
        ...
        # Frame para valores percentuais (NOVO)
        frame_percentual = ttk.Frame(frame_dados)
        frame_percentual.grid(row=3, column=0, columnspan=2, pady=5)
        
        # Percentual
        ttk.Label(frame_percentual, text="Percentual (%):*").grid(row=0, column=0, padx=5, pady=2)
        percentual_entry = ttk.Entry(frame_percentual)
        ...
        # Forma de pagamento (NOVO) para dados bancários
        ttk.Label(frame_dados, text="Forma de Pagamento:").grid(row=5, column=0, padx=5, pady=2)
        forma_pagamento = ttk.Combobox(frame_dados, values=['PIX', 'TED'], state='readonly')
        forma_pagamento.grid(row=5, column=1, padx=5, pady=2)
                ...
                if tipo_combo.get() == 'Percentual':
                    ...
                    try:
                        ...
                        valores_percentual = (
                            ...
                            "",  # Valor Total vazio para percentual
                            "",  # Parcelas vazio para percentual
                            ""   # Data inicial vazia para percentual
                        )
                        tree.insert('', 'end', values=valores_percentual, tags=('percentual',))
                    ...
                    if tem_entrada.get() and valor_entrada > 0:
                        ...
                        tree.insert('', 'end', values=valores_entrada, tags=('entrada', forma_pagto_selecionada))
                    ...
                    if valor_parcela > 0:
                        ...
                        tree.insert('', 'end', values=valores_parcelas, tags=('parcelas', forma_pagto_selecionada))
        ...
        def atualizar_campos_tipo(*args):
            ...
            if tipo_combo.get() == 'Percentual':
                frame_percentual.grid()  # Mostra frame de percentual
                frame_fixo.grid_remove()  # Esconde frame de valores fixos
            elif tipo_combo.get() == 'Fixo':
                frame_percentual.grid_remove()  # Esconde frame de percentual
    ...
    def salvar_contrato(self, num_contrato, data_inicio, data_fim, observacoes, janela):
        ...
        try:
            ...
            for item in self.tree_adm.get_children():
                ...
                dados_bancarios = self.buscar_dados_bancarios_fornecedor(cnpj_cpf, forma_pagamento)

                # Se for tipo percentual
                if valores[2] == 'Percentual':
                    # Registrar administrador com percentual no contrato
                    proxima_linha = ws.max_row + 1
                    ws.cell(row=proxima_linha, column=7, value=num_contrato.upper)
                    ...
                    ws.cell(row=proxima_linha, column=11, value=valores[3])  # Percentual
                    ws.cell(row=proxima_linha, column=12, value="")  # Valor Total vazio
                    ws.cell(row=proxima_linha, column=13, value="")  # Nº Parcelas vazio
                    
                # Se for tipo fixo e entrada
                elif valores[2] == 'Fixo' and 'entrada' in tags:
                    # Registrar administrador no contrato
                    proxima_linha = ws.max_row + 1
                    ws.cell(row=proxima_linha, column=7, value=num_contrato.upper)
                    ws.cell(row=proxima_linha, column=8, value=cnpj_cpf)
                    ws.cell(row=proxima_linha, column=9, value=nome_admin)
                    ...
                    try:
                        ...
                        hoje = datetime.now()
                        if hoje.day <= 5:
                            data_rel = hoje.replace(day=5, hour=0, minute=0, second=0, microsecond=0)
                        elif hoje.day <= 20:
                            data_rel = hoje.replace(day=20, hour=0, minute=0, second=0, microsecond=0)
                        else:
                            data_rel = (hoje + relativedelta(months=1)).replace(day=5, hour=0, minute=0, second=0, microsecond=0)
                        # Corrigir formato de data - CORREÇÃO 3
                        data_rel_str = data_rel.strftime('%d/%m/%Y')
                        
                        # Corrigir leitura da data de vencimento - CORREÇÃO 3
                        if isinstance(valores[6], str):
                            data_vencto = datetime.strptime(valores[6], '%d/%m/%Y')
                            # Garante que não tenha parte de hora
                            data_vencto = data_vencto.replace(hour=0, minute=0, second=0, microsecond=0)
                        else:
                            data_vencto = valores[6]
                            # Se for datetime, zera a parte de hora
                        ...
                        if isinstance(data_rel, datetime):
                            data_rel_limpa = data_rel.replace(hour=0, minute=0, second=0, microsecond=0)
                        else:
                            # Se por algum motivo não for datetime, converte para string e depois para datetime sem hora
                            data_rel_limpa = datetime.strptime(str(data_rel), '%d/%m/%Y').replace(hour=0, minute=0, second=0, microsecond=0)

                        ws_dados.cell(row=proxima_linha_dados, column=1, value=data_rel_limpa)
                        ws_dados.cell(row=proxima_linha_dados, column=1).number_format = 'DD/MM/YYYY'
                        ...
                        ws_dados.cell(row=proxima_linha_dados, column=5, value=f"ADM OBRA - ENTRADA - CONTRATO {num_contrato.upper()}")

                        # CORREÇÃO 2: Converter valor corretamente sem multiplicação por 100
                        try:
                            # Verificar se valores[3] é string ou número
                            if isinstance(valores[3], str):
                                valor_entrada = float(valores[3].replace(',', '.'))
                            else:
                                valor_entrada = float(valores[3])
                                
                            # Usar o valor diretamente, sem chamar formatar_valor_excel
                            cell_vr_unit = ws_dados.cell(row=proxima_linha_dados, column=7, value=valor_entrada)
                            aplicar_formatacao_celula(cell_vr_unit)
                            
                            ws_dados.cell(row=proxima_linha_dados, column=8, value=1)
                            
                            cell_valor = ws_dados.cell(row=proxima_linha_dados, column=9, value=valor_entrada)
                            aplicar_formatacao_celula(cell_valor)
                        except (ValueError, TypeError) as e:
                            print(f"Erro ao formatar valor: {str(e)}")
                            # Em caso de erro, usar 0
                            ws_dados.cell(row=proxima_linha_dados, column=7, value=0)
                            ws_dados.cell(row=proxima_linha_dados, column=9, value=0)

                        # Usar o objeto datetime para o vencimento também
                        ws_dados.cell(row=proxima_linha_dados, column=10, value=data_vencto)
                        ws_dados.cell(row=proxima_linha_dados, column=10).number_format = 'DD/MM/YYYY'
                        
                        ws_dados.cell(row=proxima_linha_dados, column=11, value='ADM')
                        
                        # CORREÇÃO 4: Adicionar dados bancários - ADICIONE ESTAS LINHAS AQUI
                        forma_pagamento = next((tag for tag in tags if tag in ['PIX', 'TED']), 'PIX')
                        dados_bancarios = self.buscar_dados_bancarios_fornecedor(cnpj_cpf, forma_pagamento)
                        ws_dados.cell(row=proxima_linha_dados, column=12, value=dados_bancarios)
                ...
                # Se for tipo fixo e parcelas
                elif valores[2] == 'Fixo' and not 'entrada' in tags:
                    # Registrar administrador no contrato
                    proxima_linha = ws.max_row + 1
                    ws.cell(row=proxima_linha, column=7, value=num_contrato.upper)
                    ws.cell(row=proxima_linha, column=8, value=cnpj_cpf)
                    ...
                    # Registrar parcelas na tabela de parcelas
                    try:
                        # Corrigir leitura da data inicial
                        if isinstance(valores[6], str):
                            data_inicial = datetime.strptime(valores[6], '%d/%m/%Y')
                        else:
                            data_inicial = valores[6]
                        ...
                        for i in range(num_parcelas):
                            ...
                            # Adicionar dados bancários à coluna de dados bancários
                            ws.cell(row=proxima_linha, column=32, value=dados_bancarios)
    ...
    def buscar_dados_bancarios_fornecedor(self, cnpj_cpf, forma_pagamento):
        """Busca os dados bancários do fornecedor conforme a forma de pagamento"""
        try:
            # Usar a função central em utils.py
            from config.utils import buscar_dados_bancarios_fornecedor as buscar_dados
            return buscar_dados(cnpj_cpf, forma_pagamento)
        except ImportError:
            # Tratar o caso em que a função ainda não está disponível em utils.py
            try:
                ...
                for row in ws.iter_rows(min_row=2, values_only=True):
                    if row[0] == cnpj_cpf:
                        ...
                        if forma_pagamento == "PIX" and row[10]:  # Chave PIX está na coluna K
                            ...
                            # Adicionar CNPJ/CPF se for TED
                            if forma_pagamento == "TED" and row[0]:
                                partes_dados.append(str(row[0]))
